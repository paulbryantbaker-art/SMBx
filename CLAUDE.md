...
**Unified type scale (LOCKED 2026-07-13 consistency pass).** The practice site (`.pd`) routes ALL text through one ladder — do not reintroduce per-section inline `fontSize`/`maxWidth` overrides. Roles: `.pd-h1` hero clamp(40,5.2vw,80) / `.pd-h2` section header clamp(34,4.4vw,54) (`.pd-cta-h` shares it) / `.pd-h3` minor header 21·700 / `.pd-sub` lede 21 / `.pd-body` 17·1.7 (the one body size) / `.pd-body-sm` 15 / `.pd-caption` 13.5 / `.pd-mono` 13.5·0.09em. Protected drama keeps its own scale: `.pd-quote` pull-quotes, `.pd-statement` in-flow rhetorical lines (below H2 by design), `.pd-num`/`.pd-num` numerals, and the self-contained Market Map document (`.pd-map`/`.map-*`, PDF-parity — never fold into site roles). Headers use ONE measure (18em); body measures may follow layout. New sections adopt these classes, never bespoke sizes.
