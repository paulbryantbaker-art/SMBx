...
compose:  bone canvas · illustration at FULL BAND HEIGHT sitting right of centre
          · a short Deal Green rule anchoring the left third.
...
work. Deep green (#0A7A58) and amber (#E8A62B) on a warm bone background
(#FCFAF6), with jade (#0FA97C) for secondary planes. Subject spread across the
...
of centre, a short Deal Green rule on the left third. Save as JPEG q88.
...
bone `#F6F4EF` and landed warmer still. `ARTWORK_LIFT` in `house/tokens.ts`
