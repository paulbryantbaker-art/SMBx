 ...
 *   cd <this workspace>
 *   npx tsx <kit>/build-deck.mts decks/<name>.deck.mts
 * (build-deck defaults to ./media + ./assets for images and ./collateral for
 *  output — no flags needed from inside the workspace.)
 *
 * Usage: npx tsx init-workspace.mts [targetDir]
 */
import { mkdirSync, writeFileSync, copyFileSync, existsSync } from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';

const ROOT = path.dirname(fileURLToPath(import.meta.url)); // the kit dir
...
// starter deck spec
const exampleSrc = path.join(ROOT, 'decks/example.deck.mts');
...
// starter plan
const planSrc = path.join(ROOT, 'posting-plan.md');
...
writeFileSync(path.join(target, 'README.md'), [
  ...
  'README.md in the studio kit.',
].join('\n'));

const builder = path.join(ROOT, 'build-deck.mts');
