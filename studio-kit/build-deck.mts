 ...
 *
 * Runs the SAME house design system (Ledger palette, Fraunces/Inter/Plex,
 * boardroom texture, the framed-cover grammar, the rasterized-PDF
 * renderer-proof pass) as pure LOCAL compute — no app, no app API key. Style,
 * fonts, and brand assets are bundled in this kit; your media comes from local
 * folders; the plan is a file.
 *
 * Usage:
 *   npx tsx build-deck.mts <spec.deck.mts> [--media <dir>] [--out <dir>]
 *
 * The spec is a small .mts file that `export const deck = {...}` (see
 * decks/example.deck.mts for the full field reference). Image paths resolve
 * against --media, then ./media, ./assets, the bundled brand/, the spec's own
 * folder, then CWD; absolute paths pass through.
 *
 * Outputs (into --out, default ./collateral): <slug>.pdf (the swipeable
 * carousel), <slug>-pNN.jpg (each page), <slug>-caption.txt (the post copy).
 */
import { readFileSync, writeFileSync, mkdirSync, existsSync } from 'node:fs';
import path from 'node:path';
import { pathToFileURL, fileURLToPath } from 'node:url';
import { fontFaceCss } from './lib/fonts.mts';
import { newRenderPage, closeBrowser } from './lib/render.mts';

// Self-contained: the kit's own brand assets + lib, no parent-repo imports.
