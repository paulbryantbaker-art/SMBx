...
**Status:** Started in repo. `/.well-known/mcp/server-card.json` and `/.well-known/mcp` now generate from the existing DEFINITIVE manifest, MCP tool inventory, substrate architecture doctrine, and THE LINE metadata. `/api/definitive/registry-package` now also carries submission-ready packages for the canonical MCP registry, third-party MCP directories, client-store candidates, enterprise allow-list templates, semantic tool metadata, and THE LINE evidence. The endpoints are public discovery only; execution still routes through authenticated/governed tool contracts.
