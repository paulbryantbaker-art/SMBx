...
- Validate the authenticated route path. **Expanded pass complete:** `npm run test:definitive-auth-route` creates a stable DB/JWT fixture and proves protected tool inventory, THE LINE inventory, corpus observation-type rules, data-rights read/grant flow, sanitized corpus observation writes, unsupported-version refusal, `compose_model_stack` mandate-chain output with G28/G29/G30 routing, pinned model-backed audit-packet retrieval, Studio export audit-packet retrieval, and staged-action list/cancel behavior.
...
1. Continue deterministic runtime functions and schemas without changing THE LINE readiness. Sixty-eight models are now executable with conformance coverage: M139, M148, M151-M160, M164-M172, and M177-M223. Eight volatile models now have research-only runtime scaffolds: M143, M161-M163, and M173-M176. Prompt/runtime language for research-only, professional-handoff, and pass-through route briefs has a first pass. Authenticated route validation now has expanded DB/JWT coverage at `npm run test:definitive-auth-route`.
...
5. Grow conformance cases from 224 toward 240 across prompt behavior, route-trigger behavior, deal-mechanics route profiles, and meta behavior.
6. Add authenticated route-level smoke tests for explicit THE LINE refusal envelopes; Studio export packet retrieval, model-backed chat packet retrieval, and staged approval list/cancel behavior now have coverage.
