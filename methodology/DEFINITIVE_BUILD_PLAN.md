...
| Conformance tests | 750 minimum | Expand coverage by model status. Production executable models need deterministic fixtures before they can leave research/planning status. Authenticated route smoke now covers JWT + mandate-chain + protected tool execution + the recursive DealState loop. |
...
- `reference/definitive-ts` and `reference/definitive-python` now start the internal open-standard proof: cloneable, DB-free reference packages with M109, M139, M199, and M206 pure functions, sample-only authorities, README/example calls, and shared fixture tests through `npm run test:definitive-reference` and `npm run test:definitive-reference-python`.
...
**Status:** Started in repo. Migration `073_definitive_authority_register.sql` creates the first Authority Register schema, seeds 50 active US authorities, and wires `lookup_citation` through the register while preserving V19 `citation_registry` compatibility. Staged ingestion now reaches 233 active seed rows: `078_definitive_authority_register_batch2.sql` adds bankruptcy/restructuring, connected tax, Treasury, FIRPTA/real-estate tax, and IP title/lien anchors; `079_definitive_authority_register_batch3.sql` adds agreement architecture, THE LINE pass-through pricing, recovery-data, digital-asset research, regulated-industry, and real-estate pass-through-standard anchors; `080_definitive_authority_register_batch4.sql` adds real-estate transfer/CITT anchors, transaction-cost and deferred-payment tax anchors, ALTA/BOMA real-estate standards, IP/OSS public-record and license anchors, software/data pass-through substrates, secondaries guidance, credit-documentation, and NVCA equity mechanics; `081_definitive_authority_register_batch5.sql` adds bankruptcy procedure depth, cramdown/1111(b) case-law anchors, deferred-payment and FIRPTA form anchors, REIT sections, and UCC Article 9 liquidation/lien mechanics. The DB-free seed plan is code-addressable at `definitive://v1.1/deal-mechanics/authority-seed-plan`, targets 920 planned entries against the 800+ requirement, and exposes bankruptcy, restructuring/LME, IRC/Treasury, real-estate, connected-tax, agreement-architecture, IP, pass-through pricing, recovery-data, digital-asset, regulated-industry, Delaware, market-data, methodology, and compliance/audit categories through the public spec and agent card.
...
- Validate the authenticated route path. **Expanded pass complete:** `npm run test:definitive-auth-route` creates a stable DB/JWT fixture and proves public MCP server-card discovery, public MCP well-known discovery, public registry/enterprise allow-list packages, public schema/runbook/model-catalog routes, protected tool inventory, THE LINE inventory, corpus observation-type rules, data-rights read/grant flow, sanitized corpus observation writes, unsupported-version refusal, explicit THE LINE refusal envelopes for human approval / counsel review / enterprise scope, the recursive DealState loop (`ingest_deal_payload` -> `update_deal_payload` -> `check_completeness` -> `compose_deal_plan` -> `diff_deal_state`), `compose_model_stack` mandate-chain output with G28/G29/G30 routing, pinned model-backed audit-packet retrieval, Studio export audit-packet retrieval, and staged-action list/cancel behavior.
...
- 400-case checkpoint is met and the suite is now at 472 cases; keep expanding across authenticated route behavior, authority depth, model-stack edge cases, public discovery, registry packaging, meta behavior, and Deal OS artifact boundaries.
...
**Status:** Started in repo. `reference/definitive-ts` exposes the first DB-free TypeScript reference package with pure functions and envelopes for M109 working-capital peg, M139 Section 1060 residual-method allocation, M199 FIRPTA withholding, and M206 indemnification ladder. It includes `README.md`, an example call, sample-only authority refs, deterministic input/output hashes, spec/methodology pins, and THE LINE boundaries. `npm run test:definitive-reference` runs 4 JSON fixture cases from `testing/definitive/reference/v1/reference-implementation.cases.json`.
...
**Status:** Started in repo. `reference/definitive-python` mirrors the TypeScript reference package for M109, M139, M199, and M206 using the same JSON fixture cases. It includes a minimal `pyproject.toml`, README, example call, sample-only authority refs, deterministic input/output hashes, spec/methodology pins, and THE LINE boundaries. `npm run test:definitive-reference-python` runs the shared 4-case fixture set from `testing/definitive/reference/v1/reference-implementation.cases.json`.
...
**Status:** In progress with the terminal schema spine now code-addressable. `definitive://v1.1/substrate-architecture` and `/api/definitive/substrate-architecture` expose eight primitives, staged MCP tool targets, the eight-axis classification key, six-phase solo-founder sequence, universal response-envelope fields, and THE LINE invariant. The schema registry now publishes `DealPayload`, `ClassificationKey`, `MissingInputContract`, `DealState`, `CompletenessSpec`, `DealReadinessLevel`, `CompletenessReport`, `DealPackage`, `PackageVerification`, `FinalizedDealPackage`, `HumanPackageRender`, `ReopenedDealPackage`, and the main agent take-back artifacts (`CapabilityCatalog`, `SourceIndex`, `SourceGapList`, `SelectiveDisclosureProof`, `ModelOutput`, `ModelExecutionHistory`, `AssumptionLog`, `OutputHash`, `AuditPacket`, `SignedManifest`, `Attestation`, `MerkleInclusionProof`). Interactive model canvas runs now persist as durable `model_executions` with canvas tab IDs, client version numbers, input/output hashes, parent-output lineage, `ModelOutput` or runtime-output envelopes, version snapshots, dependency contracts, freshness envelopes, recompute action keys, and THE LINE audit payloads. The Model Canvas now reads back saved runs in the UI and flags saved model versions as `current`, `superseded`, or `needs_rerun` when EV/purchase price, EBITDA, debt, working capital, tax, or other watched assumptions change. Today Operating Brief now reads those model executions, computes `modelRefreshNeeds`, stores that queue in the cached brief, and pushes model-rerun blockers plus recompute action keys into Today, Pipeline, Files, Studio, and deal Data Room scopes so humans and agents see stale/superseded model outputs before advancing gates, relying on files, sharing data-room artifacts, exporting collateral, or taking artifacts back into another system. Data Room document rows now hydrate linked deliverable/model provenance from `generated_from_snapshot` and `model_executions` where available, surfacing model output hashes, versions, and exact stale-match warnings on model-backed room artifacts. The authenticated agent surface exposes `list_model_executions` with optional `currentAssumptions` so external agents can ask what has already been modeled, whether a saved output is stale, which model to rerun, and which deterministic recompute action to call before advancing IOI, LOI, diligence, negotiation, close, or PMI. The authenticated route smoke now exercises public discovery plus the core recursive loop directly: MCP server-card and well-known discovery, registry/allow-list package routes, schema/runbook/model-catalog routes, partial payload ingest, DealState update with parent CID, persisted DealState read via `get_deal_state`, branch-safe `clone_deal_state`, completeness re-check, DealPlan composition, DealStateDiff take-back, portable DealPackage composition, package verification/finalization/reopen, deterministic structure permutations, computed-best vehicle under stated preferences, and persisted DealState/package control packets.
...
- In progress: expand the model dependency graph and server-side execution coverage. Durable canvas model version persistence now captures parent output hashes, version snapshots, dependency contracts, per-run freshness, and deterministic recompute plans. Cross-surface cascade invalidation has its first pass through Today Operating Brief, Today, Pipeline, Files, Studio, and deal Data Room reliance panels. Data Room rows now carry model provenance/hash labels where a filed deliverable can be linked back to a saved model execution; recompute action keys now travel with saved runs and model-refresh blockers.
...
- Add `verify_package` so receiving agents can recompute package identity, confirm companion DealState references, check take-back artifacts, and preserve THE LINE before relying on a package.
- Add `finalize_deal_package` so verified packages can emit software manifest artifacts (`AuditPacket`, `SignedManifest`, `Attestation`, `MerkleInclusionProof`) without implying closing authorization or professional advice.
- Add `reopen_deal_package` so a returned package can become a new DealState revision when an agent or human brings back fresh facts, documents, or diligence answers.
- Add the Run 20 permutation tools (`generate_permutations`, `score_permutation`, `set_objective_preference`, `compute_best_vehicle`, `expand_permutations`) so deal structure work returns a Pareto frontier and computed-best point under caller-stated preferences without becoming advice.
- Add `HumanPackageRender` to finalized packages so the same signed/audited package has a human-readable Markdown view with package IDs, verification status, audit IDs, Merkle root, next calls, and THE LINE.
...
**Status:** Started in repo. `/.well-known/mcp/server-card.json` and `/.well-known/mcp` now generate from the existing DEFINITIVE manifest, MCP tool inventory, substrate architecture doctrine, and THE LINE metadata. The tool inventory now exposes input schemas, output schema names, structured-content metadata, read-only/destructive/open-world annotations, scoped permissions, methodology pins, and no-success-fee/no-paid-human-referral declarations from the central schema map. `/api/definitive/registry-package` now also carries submission-ready packages for the canonical MCP registry, third-party MCP directories, client-store candidates, enterprise allow-list templates, semantic tool metadata, and THE LINE evidence. The endpoints are public discovery only; execution still routes through authenticated/governed tool contracts.
...
2. Continue deterministic runtime functions and schemas without changing THE LINE readiness. Sixty-eight models are now executable with conformance coverage: M139, M148, M151-M160, M164-M172, and M177-M223. Eight volatile models now have research-only runtime scaffolds: M143, M161-M163, and M173-M176. Prompt/runtime language for research-only, professional-handoff, pass-through route briefs, substrate-architecture primitives, agent no-rejection Deal OS lifecycle, iterative work surfaces, portable agent handoffs, schema-registry contracts, registry submission packages, and `CompletenessSpec` definition-of-done behavior is now covered by prompt/meta conformance cases. Authenticated route validation now has expanded DB/JWT coverage at `npm run test:definitive-auth-route`, including public discovery routes and the direct recursive DealState loop.
3. Authority Register seed plan is expanded from 500+ to 800+ and now targets 920 planned entries across bankruptcy, restructuring, IRC/Treasury, real-estate, connected-tax, agreement-architecture, IP, pass-through pricing, recovery-data, digital-asset, regulated-industry, Delaware, market-data, methodology, and compliance/audit categories. Staged ingestion now reaches 233 active seed rows: migration `079_definitive_authority_register_batch3.sql` adds active anchors for agreement economics, THE LINE pass-through pricing/referral boundaries, recovery datasets, digital-asset research, regulated industries, ASC 842, and ASTM pass-through standards; migration `080_definitive_authority_register_batch4.sql` adds CITT/transfer-tax anchors, transaction-cost and deferred-payment tax authorities, ALTA/BOMA standards, IP/OSS/license authorities, software/data pass-through substrates, secondaries, credit documentation, and NVCA equity mechanics; migration `081_definitive_authority_register_batch5.sql` adds bankruptcy procedure depth, cramdown/1111(b) case-law anchors, deferred-payment and FIRPTA form anchors, REIT sections, and UCC Article 9 liquidation/lien mechanics. Surface smoke now fails if staged Authority Register migration rows fall below the 220-row baseline.
...
6. The 400-case conformance checkpoint is met; the suite is now at 472 cases after adding Deal OS artifact coverage for lifecycle trace, IOI, LOI, data room, diligence, disclosure, document, negotiation, close, funds flow, PMI, package take-back, package verification/finalization/reopen, partial ingest/update/diff/clone, package mismatch detection, data-room overlay expansion, close/funds-flow blockers, permutation/best-vehicle behavior, MCP server-card/well-known discovery, enterprise allow-list templates, and Authority Register depth checks for bankruptcy procedure, cramdown, deferred tax, REIT, and UCC anchors. Next target is expanding toward 500 across authenticated route behavior, Authority Register depth, model-stack edge cases, public discovery/registry packages, meta behavior, and additional Deal OS artifact edge cases.
...
7. Keep extending authenticated route-level smoke coverage beyond the completed explicit THE LINE refusal envelopes for human approval, counsel review, and enterprise scope. Public MCP server-card/well-known discovery, public registry/allow-list package retrieval, public schema/runbook/model-catalog retrieval, Studio export packet retrieval, model-backed chat packet retrieval, staged approval list/cancel behavior, direct budget/plan tollgate coverage, the direct recursive DealState loop, `get_deal_state`, `clone_deal_state`, and persisted DealState control packets now have tests.
