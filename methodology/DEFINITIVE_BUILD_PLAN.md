...
6. Keep extending authenticated route-level smoke coverage beyond the completed explicit THE LINE refusal envelopes for human approval, counsel review, and enterprise scope. Studio export packet retrieval, model-backed chat packet retrieval, and staged approval list/cancel behavior now have coverage; budget/plan tollgates should be tested outside local dev entitlement bypass.
