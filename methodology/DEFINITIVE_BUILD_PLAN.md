...
**Status:** Started in repo. Migration `073_definitive_authority_register.sql` creates the first Authority Register schema, seeds 50 active US authorities, and wires `lookup_citation` through the register while preserving V19 `citation_registry` compatibility. The DB-free seed plan is now code-addressable at `definitive://v1.1/deal-mechanics/authority-seed-plan`, targets 920 planned entries against the 800+ requirement, and exposes bankruptcy, restructuring/LME, IRC/Treasury, real-estate, connected-tax, agreement-architecture, IP, pass-through pricing, recovery-data, digital-asset, regulated-industry, Delaware, market-data, methodology, and compliance/audit categories through the public spec and agent card.
...
3. Authority Register seed plan is expanded from 500+ to 800+ and now targets 920 planned entries across bankruptcy, restructuring, IRC/Treasury, real-estate, connected-tax, agreement-architecture, IP, pass-through pricing, recovery-data, digital-asset, regulated-industry, Delaware, market-data, methodology, and compliance/audit categories. Staged ingestion has now moved beyond the first 50 seeded rows: migration `078_definitive_authority_register_batch2.sql` adds 50 more active anchors for G28 bankruptcy/restructuring, G29 LME/capital-structure, connected transaction tax, Treasury regulations, FIRPTA/real-estate tax, and IP title/lien mechanics. Surface smoke now fails if staged Authority Register migration rows fall below the 100-row baseline.
...
6. Grow conformance beyond the 384-case checkpoint toward 400 across substrate schemas, prompt behavior, route-trigger behavior, model-stack behavior, deal-mechanics route profiles, and meta behavior.
