...
- Validate the authenticated route path. **First pass complete:** `npm run test:definitive-auth-route` creates a stable DB/JWT fixture and proves the protected `compose_model_stack` API returns the mandate chain plus G28/G29/G30 route map.
...
5. Extend authenticated route-level validation beyond the first DB/JWT smoke to `/api/definitive/line/inventory`, corpus rights, audit packets, and protected packet retrieval. DB-free surface validation currently passes.
6. Grow conformance cases from 214 toward 240 across prompt behavior, route-trigger behavior, deal-mechanics route profiles, and meta behavior.
