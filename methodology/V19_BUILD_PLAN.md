...
| 7 | Artifact Schemas + Thin MCP Contract | 🟡 Started: `shared/v19Artifacts.ts` defines Studio, source, model-run, audit, deal-state, and gate-state artifact contracts; `v19ResourceContract.ts` exposes first resource templates and tool contracts through the agent card. | Wire these contracts into actual resource reads/routes and keep them aligned with Studio/model/audit persistence. |
...
- Create actual resource read routes over existing server services; keep the contract internal/local until stable.
