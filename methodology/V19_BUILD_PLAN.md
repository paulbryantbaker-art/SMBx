...
|---|---|---|---|
| 1 | Pitch Book Studio V1 | 🟡 Implemented as first pass: `studio_books`, `studio_book_versions`, `studio_sources`, `studio_exports`, server routes, Studio UI, local fallback, PPTX/PDF export, five pitch-book tools, source-card-aware draft generation, and stricter Studio readiness checks exist. | Finish final Apple Glass + Neo visual polish and run live DB migration verification on Railway/local DB. |
| 2 | Pitch Book Generation Engine | 🟡 Deterministic templates for seven formats exist in `server/services/pitchBookStudio.ts`; tools exist in `server/services/tools.ts`; Studio refresh calls canonical V19 model execution, stores missing-input state/output hashes, and now pulls linked source-card facts into draft slides. | Replace remaining placeholder narrative slots with deeper file extraction and format-specific story logic. |
| 3 | Pitch Book Source Grounding | 🟡 Slide-level provenance, warning states, source cards, audit appendices, export citation validation, Studio model-health UI, and Studio V19 readiness checks exist. Readiness now catches missing model refs, citation refs without source cards, fact refs without deal/source evidence, and unsupported metrics. | Extend strict readiness blocking to every future share/publish surface beyond export. |
| 4 | V19 Foundation Repair | 🟡 `market_data_cache` collision is addressed by 067/070 compatibility migrations; V19 tables exist; `npm run verify:v19-schema` now verifies required migration/table/column coverage and can run live DB checks when `DATABASE_URL` is present. | Run the verifier against fresh and existing Railway/local databases after env access is available. |
| 5 | Canonical Server-Side V19 Runtime | 🟡 Model registry canonicalizes IDs to `MODEL.*.v1`; `v19ModelRuntime.ts` adds deterministic execution, missing-input reporting, output hashes, audit payloads; `model_executions` persists canonical runs from Studio refresh and the `execute_model` tool; Studio refresh/export writes V19 audit records; authenticated chat now writes automatic V19 audit records for model-backed deal responses. | Attach model-execution ids to downloadable audit packets/share flows. |
| 6 | Tier-0 Server Models | ✅ Full first-pass V19 server model catalog now returns deterministic outputs or explicit missing-input states: SDE, EBITDA, valuation triangulation, DSCR stress, NWC peg, sources/uses, SBA, HSR, QoE Lite, tax, legal halt scan, LBO LMM, PPA, rollover, earnout, structure analysis, buyer fit, deal score, market context, sensitivity, deal comparison, cap table, covenant, DCF, PMI, deal-kill probability, and timeline. `npm run test:v19-models` covers 25 deterministic fixtures. | Deepen formulas with live source extraction, market-data refreshes, and Excel round-trip assumptions. |
| 7 | Artifact Schemas + Thin MCP Contract | 🟡 `shared/v19Artifacts.ts` defines Studio, source, model-run, audit, deal-state, and gate-state artifact contracts; `v19ResourceContract.ts` exposes resource templates/tool contracts, including `methodology://yulia-prompts/v4`; `/api/v19/resource-contract` and `/api/v19/resources?uri=...` read authenticated internal resources. | Keep resource reads aligned with new objects and graduate the internal contract into MCP/API packaging only after it stabilizes. |
...
| 12 | Credit-budget pricing recast + real-time meter UI | PDF Agent Economy | 🟡 Server meter/entitlements started; Settings/Studio meter UI and pricing comparison language added; org pooling pending |
...
- Extend persisted model-execution records into downloadable audit packets.
...
- ✅ Add strict Studio reference checks for missing model refs, missing citation source cards, unsupported metrics, and fact refs without source evidence.
- ✅ Add `methodology://yulia-prompts/v4` as a readable internal resource.
...
- ✅ Add pricing-page/user-visible meter UI.
