...
const { useState: v3sS } = React;

function V3Sidebar({ activeDeal, onPick, onSlash, onCommand }) {
  const [q, setQ] = v3sS("");
  const deals = [
    ...
    <aside style={vsb.rail}>
      <div style={vsb.brand}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={vsb.brandText}>smbx<span style={{ color: "var(--accent)" }}>.</span>ai</span>
        </div>
        ...
        <div style={vsb.section}>
          ...
          {deals.map((d, i) => (
            ...
            >
              <span style={{
                ...vsb.dealTag,
                color: d.live ? "var(--accent)" : "var(--ink-3)",
                borderColor: d.live ? "var(--accent-ring)" : "var(--line-2)",
              }}>
                {d.tag}
              </span>
              ...
              <span className="mono" style={{ fontSize: 10, color: d.live ? "var(--accent)" : "var(--ink-4)" }}>
                {d.meta}
              </span>
            </button>
          ))}
        ...
        <div style={vsb.section}>
          ...
          {[
            ...
            <button key={cmd} onClick={() => onSlash(cmd)} style={vsb.slashItem}>
              <span className="mono" style={{ color: "var(--accent)", fontSize: 11, width: 70 }}>{cmd}</span>
      ...
      <div style={vsb.foot}>
        <div style={vsb.footRow}>
          <button style={vsb.footLink} onClick={() => onSlash("/pricing")}>
            <span className="mono" style={{ color: "var(--accent)" }}>/pricing</span>
          </button>
          <button style={vsb.footLink} onClick={() => onSlash("/how")}>
            <span className="mono" style={{ color: "var(--accent)" }}>/how</span>
          </button>
          <button style={vsb.footLink} onClick={() => onSlash("/about")}>
            <span className="mono" style={{ color: "var(--accent)" }}>/about</span>
          </button>
        </div>
...
// Document tab strip — minimal, just like an IDE
function V3DocTabs({ tabs, active, onPick, onClose }) {
  return (
    <div style={vdt.bar}>
      <div style={vdt.left} className="thin-scroll">
        {tabs.map((t) => (
          <button key={t.id} onClick={() => onPick(t.id)} style={{
            ...
            <span className="mono" style={{ fontSize: 9.5, color: t.id === active ? "var(--accent)" : "var(--ink-4)", marginRight: 8 }}>
              {t.tag}
            </span>
            <span>{t.label}</span>
            {t.modified && <span style={{ marginLeft: 7, color: "var(--accent)", fontSize: 16, lineHeight: 0 }}>·</span>}
            {t.closable && (
              <span
                role="button"
...
const vdt = {
  ...
  tabActive: {
    background: "var(--surface)",
    color: "var(--ink)",
    boxShadow: "inset 0 -1px 0 var(--surface), inset 0 2px 0 var(--accent)",
  },
  tabClose: {
    marginLeft: 8,
    width: 14, height: 14,
...
Object.assign(window, { V3Sidebar, V3DocTabs });
