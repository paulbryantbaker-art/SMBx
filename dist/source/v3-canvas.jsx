/* V3 — canvas pane: shows different content based on mode.
   ...
   Explore → static finished deal. Start → empty grid. */
const { useState: v3pS, useEffect: v3pE } = React;

function CanvasPane({ mode, demoStep }) {
    ...
    <div style={vp.wrap}>
      <div style={vp.head}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {!isLearn && (
            <span className="mono" style={{ fontSize: 9.5, color: "var(--accent)", marginLeft: 6 }}>
              ● modified
            </span>
          )}
          {isLearn && (
      ...
      <div style={vp.foot}>
        {isLearn ? (
          ...
          <>
            ...
            <span className="mono" style={{ fontSize: 10, color: "var(--accent)" }}>PURSUE</span>
            <div style={{ flex: 1 }} />
...
function DealCanvas({ mode, demoStep }) {
...
function KpiCell({ label, val, accent, warn, dim }) {
  return (
    <div style={vp.kpiCell}>
      ...
      <div className="mono" style={{
        ...
        letterSpacing: "-0.02em",
        color: dim ? "var(--ink-4)" : accent ? "var(--accent)" : warn ? "var(--warn)" : "var(--ink)",
        marginTop: 4,
...
function Sec({ n, title, status, children }) {
  return (
    <section style={vp.sec}>
      <div style={vp.secHead}>
        ...
        {status === "done" && (
          <span className="eyebrow eyebrow-accent" style={{ fontSize: 9 }}>✓ done</span>
        )}
      </div>
      ...
      {rows.map((r, i) => {
        ...
        return (
          <div key={i} className="mono" style={{
            ...
            fontSize: 12,
            background: primary ? "var(--accent-soft)" : sub ? "var(--surface-2)" : "transparent",
            color: primary ? "var(--accent)" : sub ? "var(--ink)" : "var(--ink-2)",
  ...
  return (
    <div className="fade-up">
      <div style={vp.baseRow}>
        ...
        <div style={vp.bar}>
          <div style={{ ...vp.barFill, width: "55%", background: "var(--accent)" }} />
      ...
      <div style={vp.baseRow}>
        ...
        <div style={vp.bar}>
          <div style={{ ...vp.barFill, width: "8%", background: "var(--accent)" }} />
          <span className="mono" style={{ ...vp.barLabel, left: "calc(8% + 8px)", color: "var(--accent)" }}>+$340K after-tax</span>
        </div>
      </div>
      <div style={{ marginTop: 14, padding: 10, background: "var(--surface-2)", border: "1px dashed var(--line-2)", borderRadius: 6 }}>
        <span className="eyebrow eyebrow-accent" style={{ fontSize: 9 }}>YULIA · NOTE</span>
...
if (!document.getElementById("v3-spin")) {
...
Object.assign(window, { CanvasPane });
