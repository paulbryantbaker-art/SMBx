...
|----------|----------------|--------|
| **This file (BUILD_PLAN_v6.md)** | Everything: roadmap, tech stack, pricing, intelligence engine, conversion flow, capital access strategy, competitive positioning | MASTER |
...
| **Capital Access Strategy Report** | Full buyer/seller financing landscape, competitor analysis, monetization blueprint, regulatory requirements | REFERENCE (distilled into BUILD_PLAN_v6 Phases B/D/G/H/I/J) |
...
SMBX is an AI-powered M&A intelligence platform where a conversational AI advisor named Yulia guides users through buying, selling, or getting agency-level deal work at any deal size — from a $300K landscaping company to a $500M PE roll-up. Users talk to Yulia, not dashboards. She asks questions, classifies their business, runs financial analysis, generates institutional-grade deliverables, surfaces market intelligence, optimizes capital structures in real-time, and advances them through a structured deal process. The platform monetizes through a pure wallet system ($1 = $1) where users pay per deliverable at ~10% of human advisory cost, plus lender referral fees on funded deals (1.5–2.5% of loan amount). The intelligence engine — powered by free government data (Census, BLS, FRED, SEC EDGAR, SBA Lender Activity Reports, FDIC) and Claude's research capabilities — delivers PitchBook-quality market intelligence and data-backed lender matching for under $225/month in infrastructure costs. No existing platform combines AI-synthesized market intelligence with deal flow and acquisition-specific capital matching. That gap is the moat.

**The Product Is Not Documents. The Product Is Intelligence + Capital Access.**

Users come for a valuation, stay for the market intelligence, and buy deliverables when the intelligence surfaces something actionable. The LOI → DD → Close workflow is table stakes. The WOW factor is: "I opened the app and Yulia told me a business matching my buy box just appeared, it's undervalued by 15%, the owner has no succession plan — and here's your optimal capital stack with the three lenders most likely to fund this deal."

---

## 2. Current State (February 25, 2026)
...
| B1 | Deal Sourcing | Mixed | Sourcing Sprint ($60), Target Fit Scoring (Free), **SBA Bankability Report ($75)** |
...
### INTELLIGENCE MATURITY MODEL — What Powers Yulia at Each Phase

The public pages describe the FULL platform vision. At launch, Yulia delivers real value using Claude's substantial M&A training knowledge. Each post-launch phase layers real-time data on top, making the same features more precise and sourced.

| Capability | Phase A-E (Launch) | Phase G+ (Data-Enriched) |
|---|---|---|
| Business classification + NAICS | Claude's training knowledge | Census CBP real establishment counts |
| Add-back identification | Claude knows common add-backs by industry | IRS SOI benchmarks validate amounts (Phase I) |
| Valuation ranges | Claude knows market multiples by sector/size | Real comps from EDGAR + BizBuySell (Phase I) |
| SBA bankability | Claude knows SOP 50 10 8 rules + structures | Live SOFR rates from FRED recalculate DSCR daily |
| Market intelligence | Claude synthesizes from training data | Census + BLS + BEA + FRED real-time data |
| Seven-factor scoring | Claude evaluates qualitatively | Structured scoring with data-backed thresholds |
| Industry timing (PE roll-ups) | Claude knows recent consolidation trends | GDELT + news event detection in real-time (Phase H) |
| Geographic adjustments | Claude knows general market differences | BEA Regional Price Parities with exact multipliers |

**This is honest and valuable at every stage.** A Claude-powered Yulia with deep M&A prompts already outperforms most human advisors at the Main Street level. The data layers make her institutional-grade.
...
**Dock Tools — What Each Does at Launch:**
- [ ] **Upload financials** → triggers file upload flow (built properly in Phase B)
- [ ] **Business valuation** → pre-fills input, Yulia handles via conversation + seven-factor model (Phase B)
- [ ] **Search for a business** → pre-fills input, Yulia helps define criteria and evaluates opportunities user brings. NOTE: No listing data source at launch — Yulia helps frame the search, not execute it. Phase G adds market scanning against defined criteria. Future: BizBuySell/Acquire.com API integration if available.
- [ ] **SBA loan check** → pre-fills input, Yulia runs SBA analysis using training knowledge (Phase B adds structured report, Phase G adds live FRED rates)
...
**Goal**: Yulia drives users through gates; anonymous → authenticated transition; structured scoring
**Effort**: ~16 hours
...
**Seven-Factor Scoring Model (referenced across sell + buy journeys):**
- [ ] Structured scoring system: recurring_revenue_pct, customer_concentration_pct, owner_dependency (1-5), growth_rate_pct, margin_quality (1-5), financial_cleanliness (1-5), industry_timing (1-5)
- [ ] Each factor scored 1-10 with specific thresholds (e.g., recurring revenue >50% = 8+, <20% = 3)
- [ ] Composite score maps to multiple range adjustment (e.g., score 70+ = top of range, score 30 = bottom)
- [ ] Stored in deal record as `seven_factor_scores` JSONB
- [ ] At launch: Claude evaluates from conversation + financials. Phase G+: data-backed thresholds from IRS/Census
- [ ] Yulia presents scorecard to user: "Here's where your business stands on the seven factors that determine your multiple."

**Document Upload + Processing Pipeline:**
- [ ] File upload endpoint: accept PDF, XLSX, CSV (P&Ls, tax returns, balance sheets)
- [ ] Document parsing: extract structured financial data (revenue, expenses, line items)
- [ ] For launch: Claude processes uploaded documents via multimodal (PDF → text extraction → structured data)
- [ ] Parsed data populates deal financial fields automatically
- [ ] Yulia confirms: "I've extracted your financials. Revenue $X, total expenses $Y. Let me check for add-backs."

**SBA Bankability + Capital Structure Advisory (dock tool #4 + buy journey core feature):**
- [ ] SBA eligibility check: SOP 50 10 8 rules (size standards, industry exclusions, citizenship, criminal history)
- [ ] DSCR calculation: net cash flow / annual debt service (lenders require 1.15×-1.25×)
- [ ] Global DSCR: include buyer's personal income + existing debt for complete picture
- [ ] At launch: use Claude's knowledge of current rate environment (update SBA rate range in system prompt monthly)
- [ ] Phase G: live SOFR rates from FRED API replace static ranges
- [ ] Equity injection modeling: 10% minimum, buyer injection + seller note structure
- [ ] Seller note standby modeling: full standby for entire loan term (2025 SBA rule)
- [ ] ROBS (Rollover for Business Startups) modeling: flag when buyer has $50K+ in 401(k)/IRA, model C-Corp setup + Guidant-style admin costs
- [ ] Add deliverable: **SBA Bankability Report ($75)** to BUY journey B1 gate
- [ ] Capital stack recommendation engine (decision tree, not AI): deal size → SBA tier check → conventional check → mezzanine check → alternative check → recommended stack with percentages and dollar amounts
- [ ] Buyer financial profile collection (conversational, not a form): credit score range, liquid assets, retirement accounts, home equity estimate, citizenship, industry experience years — stored in deal record
- [ ] Yulia's system prompt includes capital structure advisory as CORE competency: SBA rules, seller financing norms (60-80% of SMB deals), ROBS mechanics, earnout structures, working capital needs

**Capital Stack Decision Tree (deterministic logic, not AI judgment):**
```
Tier 1: deal_size ≤ $5M AND buyer_credit ≥ 690 AND DSCR ≥ 1.25x → SBA 7(a) primary
Tier 2: deal includes real estate → SBA 504 overlay for property + 7(a) for goodwill
Tier 3: $5M < deal_size ≤ $25M → Conventional senior (50-70%) + mezzanine (15-25%) + equity
Tier 4: $25M < deal_size ≤ $50M → Senior bank (40-60%) + private credit/mezzanine (15-25%) + PE equity (20-40%)
Tier 5: $50M < deal_size ≤ $250M → Syndicated credit facility or unitranche (40-55%) + sub debt/second lien (10-20%) + sponsor equity (30-45%)
Tier 6: $250M < deal_size ≤ $1B → Syndicated leveraged loan + high-yield bond (50-65%) + sponsor equity (35-50%). Club deals likely.
Tier 7: deal_size > $1B → Full capital markets: multi-tranche debt (term loan + revolver + bonds) + megafund sponsor equity. Consortium bids. Flag: "You'll need an investment bank and legal counsel for execution — I can model the structure and prepare analysis."
Alt:  buyer_credit < 690 OR limited collateral → ROBS + heavy seller financing + earnout
Equity injection sources (priority): cash savings → HELOC → ROBS → investor equity (Tiers 1-3 only)
```
Note: Tiers 1-4 use deterministic code. Tiers 5-7 defer to Yulia's conversational intelligence (Claude's training knowledge covers institutional M&A extensively). The decision tree routes to the right tier; Yulia provides the nuanced structuring advice within each tier.

**Verify**: Full S0 intake conversation → seven-factor scores populated → gate advances to S1 → upload P&L → financials extracted → add-backs identified → gate advances to S2 → paywall appears. Deal record has all fields + scores populated. SBA check works from dock tool.
...
### Phase D: Deliverable Generation Pipeline + Worker Service
**Goal**: AI generates institutional-grade deliverables via background processing
**Effort**: ~16 hours

**Worker Service Deployment (moved from Phase G — needed now):**
- [ ] Deploy **Worker service** on Railway (separate service, same repo, `node dist/worker.js`)
- [ ] Install **pg-boss** as PostgreSQL-native job queue
- [ ] Web service enqueues generation jobs → Worker service processes them
- [ ] Shared PostgreSQL via Railway private networking
- [ ] This establishes the infrastructure pattern that Phase G expands for intelligence

**Deterministic Backbone + Selective Intelligence Pattern (from CrewAI production insights):**
Simple code handles 70% of the deliverable workflow; AI handles the 30% requiring judgment:
- [ ] Deterministic: job queue management, status tracking, template selection, data assembly, PDF/XLSX export
- [ ] AI (selective): financial analysis, narrative writing, valuation reasoning, risk assessment, recommendations
- [ ] Each deliverable has a structured template + AI-generated content sections
- [ ] This prevents AI hallucination on structure while leveraging reasoning for insights

**Generation Pipeline:**
- [ ] Model routing: Haiku (classification) → Sonnet (analysis) → Opus (premium docs)
- [ ] Generation flow: purchase → pg-boss job enqueued → Worker picks up → AI builds content → status 'complete'
...
**Capital Structure Analysis Deliverable ($125, BUY B2):**
- [ ] Template sections: recommended capital stack (SBA/conventional/mezzanine/equity split with dollar amounts), DSCR sensitivity table at 3 interest rate scenarios (current, +1%, +2%), equity injection source waterfall, monthly debt service schedule (Year 1-5), seller note structure recommendation, total cost of capital
- [ ] Deterministic backbone: 70% structured math (DSCR calc, amortization tables, injection modeling)
- [ ] AI sections: narrative on financing strategy, risk factors, lender positioning advice, negotiation leverage points
- [ ] Ready for future enrichment: template has placeholder sections for "Top Lenders for This Deal" (populated when SBA Activity Report data available in Phase G) and "Apply Now" CTA (populated when lender partnerships active)

**Verify**: User purchases valuation → pg-boss job created → Worker generates report (30-60 sec) → PDF available → Yulia summarizes findings in chat. Worker process runs independently from web server.
...
**Infrastructure — Expand Existing Worker for Intelligence:**
- [ ] Worker service already deployed from Phase D — expand with intelligence agent jobs
- [ ] Add pg-boss cron scheduling for nightly data refresh (`0 2 * * *`)
- [ ] Add pg-boss event-driven triggers: new deal added, data source updated, threshold crossed
...
- [ ] Migrate SBA rate data from static system prompt to live FRED API (replaces Phase B workaround)
- [ ] Seven-factor scoring thresholds now data-backed (IRS/Census replaces Claude estimates from Phase B)
...
**Capital Access Intelligence Layer (makes platform lender-partner-ready):**
- [ ] **SBA Lender Activity Reports** (sba.gov, monthly CSV): ingest loan approvals by state and lender. Build lender × NAICS × geography activity matrix. Answers: "which lenders fund pest control acquisitions in Texas?"
- [ ] **FDIC BankFind Suite API** (api.fdic.gov, no key required): every FDIC-insured institution — assets, deposits, geographic coverage, financial health. Cross-reference with SBA data to identify active acquisition lenders by region
- [ ] **CDFI Fund database** (cdfifund.gov): 1,400+ certified CDFIs searchable by state and program. Relevant for buyers with credit challenges (CDFIs accept 620 score, $50K minimum revenue)
- [ ] Lender ranking algorithm: SBA approval volume × geographic match × NAICS match × deal size fit = lender_score
- [ ] Enrich Capital Structure Analysis deliverable with "Top 5 Lenders for This Deal" section (real data replaces placeholder from Phase D)
- [ ] Lender intelligence as FREE conversational feature: when Yulia discusses financing, she naturally names the most active lenders for this deal type + geography. Not a deliverable — just Yulia being smart. Users can't Google "which SBA lenders funded pest control acquisitions in Dallas last quarter" but Yulia knows because she has the data.
- [ ] Live SOFR rates from FRED replace static system prompt values (SBA Bankability Report now uses real-time rates)
- [ ] SBA program monitoring: track rule changes, guarantee fee updates, SBSS score requirements via FRED + SBA press releases
...
### Phase H: Active Deal Intelligence + Event Detection + Capital Access Revenue (Month 3)
**Goal**: Monitor live deals, detect market events, intervene proactively; activate lender partnerships
**Effort**: ~20 hours | **Revenue Impact**: +$200-$500 per active deal + referral fee revenue
...
**Capital Access Partnership Infrastructure (make-ready for lender revenue):**
- [ ] **Lendio Embedded Marketplace** integration: white-labeled lending experience, single embed, 75+ lender partners. User completes capital structure analysis → "Apply Now" button → Lendio flow opens. Minimal engineering — they provide the embed code.
- [ ] **Guidant Financial affiliate**: ROBS referral link when Yulia recommends ROBS as equity injection source. $4,995 setup = meaningful referral commission.
- [ ] **SmartBiz Partners** signup: SBA loan referrals with commission tracking portal. 2% referral fee + 2% packaging fee on SBA loans.
- [ ] Referral tracking system: deal_id → lender_partner → referral_date → click_through → application_status → funded_amount → fee_earned. Tracks attribution from Yulia's recommendation through funding.
- [ ] Capital access CTA injection: when Yulia delivers a Capital Structure Analysis or SBA Bankability Report, the deliverable includes a "Next Step: Apply" section with partner links
- [ ] Compliance framework: clear disclaimers ("smbx.ai is not a lender"), SBA Form 159 disclosure guidance for broker fees, no dual compensation (never charge buyer AND receive lender fee for same service)
- [ ] GLBA baseline: privacy policy updated for financial data collection/transmission, encrypted data at rest and in transit (Railway already provides this via TLS + managed PostgreSQL encryption)

---

### Phase I: Full Intelligence + Deep Data + Direct Lender Relationships (Month 4)
**Goal**: Complete the intelligence loop — deep data, public comps, portfolio analytics; scale capital access revenue
**Effort**: ~20 hours
...
**Direct Lender Partnership Program (requires 10+ active buyer deals for leverage):**
- [ ] Outreach to 5-10 SBA Preferred Lenders identified from SBA Activity Reports (Live Oak Bank, Celtic Bank, Ameris Bank, regional leaders)
- [ ] Value proposition: pre-qualified M&A buyer leads with full deal context (LOI, target financials, buyer profile, DSCR pre-calc) — far higher quality than general business loan leads
- [ ] Negotiate 1.5–2.5% referral fees (lender-paid), up from 0.5–1% affiliate rates
- [ ] Direct API integration where available: submit pre-qualified application data, receive status updates, track through funding
- [ ] Equipment financing partner (for deals with fleet/equipment component)
- [ ] Working capital facility partner (buyer needs revolving line from day one post-close)
- [ ] Lender intelligence gets richer as direct partnerships provide approval rate feedback — Yulia can say "Celtic Bank has funded 3 deals like yours this month and their approval rate for pest control is 78%"

---

### Phase J: Intelligence Flywheel + Research Engine + Capital Matching Engine (Month 6+)
**Goal**: Platform intelligence compounds with every deal; capital access becomes second-largest revenue line
**Effort**: Ongoing
...
**Capital Matching Engine (the killer feature — real-time capital stack optimization):**
- [ ] Real-time capital stack recalculation: deal terms change → Yulia instantly remodels the financing structure
  - Seller note terms change? → Recalculate equity injection gap, suggest alternatives
  - Appraisal comes in low? → Restructure with earnout, renegotiate price, model new DSCR
  - Seller won't accept full standby? → Flag SBA injection eligibility killed, model all-cash injection requirement
- [ ] ML-powered lender matching: historical outcome data (which lenders funded which deal profiles) improves recommendations over time
- [ ] Lender preference learning: track approval/decline patterns by NAICS, geography, deal size, buyer profile → predict approval probability per lender
- [ ] Capital advisory as premium service for deals >$5M: charge 1-2% success fee on capital raised (Modified Lehman for >$10M), on top of lender referral fees
- [ ] Capital access dashboard: all active deals with financing status, lender touchpoints, application progress, expected close dates
- [ ] R&W Insurance introductions for deals >$10M: partner with licensed broker, flat referral fee per introduction
- [ ] Post-funding working capital facility matching: day-one revolving credit line for buyer (10-20% of annual revenue, secured by receivables)

**Revenue at Scale:**
Capital access referral fees at 3-5 funded deals/month × $1.5M avg deal × 1.5% blended fee = **$810K-$1.35M/year**. This becomes 20-40% of total platform revenue alongside advisory deliverables and wallet purchases. Per-deal effort is near-zero once infrastructure exists — every M&A deal already needs financing.
...
|-------|------|------|--------|------------|
| A | Chat Morph + Anonymous | NOW | ~12 hrs | SPA shell, dock, journey views, chat morph, anonymous sessions |
| B | Gates + Scoring + Docs + Capital Stack | Week 1-2 | ~22 hrs | 22 gates, seven-factor model, doc upload/parsing, SBA analysis, capital stack decision tree, buyer profile, auth |
| C | Wallet + Stripe + Paywall | Week 2-3 | ~16 hrs | Payment system, menu pricing, paywall at S2/B2/R2 |
| D | Deliverables + Worker | Week 3-4 | ~18 hrs | pg-boss worker service, AI generation pipeline, PDF/XLSX export, Capital Structure Analysis deliverable |
...
| G | Market Watch + Data Pipeline + Capital Intel | Month 2 | ~28 hrs | LangGraph.js agents, gov data APIs, SBA Lender Activity Reports, FDIC/CDFI data, lender ranking, engagement tiering, daily briefings |
| H | Deal Intel + Events + Capital Partnerships | Month 3 | ~20 hrs | Deal pacing, GDELT news, event scoring, Lendio embed, Guidant/SmartBiz affiliates, referral tracking |
| I | Full Intelligence + Deep Data + Direct Lenders | Month 4 | ~20 hrs | SEC EDGAR, IRS SOI, BizBuySell, pgvector, portfolio analytics, direct lender partnerships, enhanced lender matching |
| J | Flywheel + Research + Capital Engine | Month 6+ | Ongoing | Multi-agent research, data products, real-time capital stack optimization, ML-powered lender matching, capital advisory services |

**Total to launch**: ~80 hours (~5 weeks)
**Total to full intelligence + capital platform**: ~160 hours (~6 months post-launch)
...
**SBA Lender Activity Reports:** Monthly CSV downloads from sba.gov showing loan approvals by state and lender — 800+ lenders, 78,000+ 7(a) loans in FY2025. Reveals exactly which lenders are most active for acquisition loans by geography and industry. Top lenders: Live Oak Bank ($2.8B+ FY2025), Huntington National Bank (largest by count), Northeast Bank (7,800 loans FY2025). Powers the lender ranking algorithm.

**FDIC BankFind Suite API:** Data on every FDIC-insured institution — assets, deposits, geographic coverage, financial health. REST API at api.fdic.gov, no registration required. Cross-referenced with SBA data to identify community banks active in small business acquisition lending by region.

**CDFI Fund Database:** 1,400+ certified Community Development Financial Institutions searchable by state and program at cdfifund.gov. CDFIs serve underserved communities with relaxed qualifying criteria (620 credit score, $50K minimum revenue). Fills gaps for buyers who don't qualify for traditional SBA lending.
...
| AI capital stack optimization | ❌ | ❌ | ❌ | ❌ | ✅ |
| Embedded SBA prequalification | ✅ (LoanBud) | ❌ | ❌ | ❌ | ✅ (Phase H) |
| Data-backed lender matching | ❌ | ❌ | Relationship | ❌ | ✅ (Phase G) |
| Real-time deal restructuring | ❌ | ❌ | ❌ | ❌ | ✅ (Phase J) |

**Capital Access Competitive Landscape:**
- **BizBuySell**: LoanBud partnership (listing-level SBA prequal) + iBusiness Funding ($7B+ SBA processed). Volume play — 120K listings, 50M visits. No AI matching.
- **Flippa**: Broadest ecosystem — Guidant (ROBS), Unsecured Funding Source ($500M+), Flippa Invest (Reg D equity raising). But focused on digital/micro businesses.
- **Acquire.com**: Pipe partnership (dated — Pipe pivoted), eCommerce Lending for SBA. LOI builder includes seller financing terms.
- **Baton Market**: $15.5M raised, 70% close rate claim, lender partnerships, QuickBooks integration. Closest competitor in spirit.
- **Lendio**: Best partnership infrastructure — Embedded Marketplace, 75+ lenders, $16B+ funded, 68% approval rate. White-label ready.
- **SMBX differentiator**: Only platform where the AI advisor knows the full deal context (target financials, buyer profile, seller terms, seven-factor scores) and uses it to optimize the capital stack in real-time as deal terms evolve. This is not matching — it's structuring.
...
### Capital Access Tables (build Phase G, revenue Phase H+)
- **lenders** (lender_id, name, type [sba_preferred|sba_express|conventional|cdfi|mezzanine|equipment], geographic_coverage JSONB [state_codes], naics_specialties JSONB, min_deal_size, max_deal_size, is_active, partner_tier [affiliate|direct|none], partner_since, contact_info JSONB)
- **lender_activity** (lender_id, naics_code, geography_fips, loan_count, total_volume_cents, avg_deal_size_cents, period, source [sba_activity_report|fdic|manual], updated_at) — populated from SBA monthly CSVs and FDIC API
- **capital_recommendations** (deal_id, recommended_stack JSONB [{source, type, amount_cents, rate_range, term_months, notes}], lender_matches JSONB [{lender_id, match_score, reason}], dscr_scenarios JSONB, created_at, updated_at) — generated by capital stack decision tree
- **referral_tracking** (id, deal_id, lender_id, partner_type [lendio|guidant|smartbiz|direct], referral_date, click_through_at, application_submitted_at, funded_at, funded_amount_cents, fee_amount_cents, fee_paid_at, status [referred|applied|underwriting|funded|declined|expired])

### Buyer Profile Fields (add to deals table, Phase B)
- buyer_credit_score_range VARCHAR(20) — '690-720', '720-750', etc. (never store exact scores)
- buyer_liquid_assets_cents BIGINT
- buyer_retirement_funds_cents BIGINT — for ROBS modeling
- buyer_home_equity_cents BIGINT — for HELOC injection modeling  
- buyer_citizenship_status VARCHAR(20) — 'us_citizen', 'permanent_resident', 'other'
- buyer_industry_experience_years INTEGER
- buyer_existing_debt_annual_cents BIGINT — for global DSCR calculation
- seller_financing_willingness VARCHAR(20) — 'yes', 'no', 'negotiable', 'unknown'
- seller_standby_willingness VARCHAR(20) — 'full_standby', 'partial', 'no', 'unknown'
...
# SBA Lender Activity Reports — no key required (CSV download from sba.gov)
# FDIC BankFind Suite — no key required (api.fdic.gov)
# CDFI Fund — no key required (cdfifund.gov)

# Capital Access Partnerships (Phase H+)
# LENDIO_PARTNER_ID=... (embedded marketplace integration)
# GUIDANT_AFFILIATE_ID=... (ROBS referral tracking)
# SMARTBIZ_PARTNER_KEY=... (SBA loan referral portal)
```
...
| Metric | Transaction Only | With Intelligence | With Intelligence + Capital Access |
|--------|-----------------|-------------------|------------------------------------|
| Gross margin | 95%+ | 95%+ | 95%+ (referral fees are pure margin) |
| L1 annual revenue per user | $500–$2,870 | $800–$4,070 | $800–$4,070 + referral fees on funded deals |
| L3 annual revenue per user | $6,000–$50,000 | $6,800–$46,700 | $6,800–$46,700 + $15K-$75K per funded deal |
| Year 1 ARR target | $1.58M | $2.0M | $2.0M + $810K–$1.35M capital referrals |
| Year 5 ARR target | $12.8M | $18.7M | $18.7M + $4.8M–$9.6M capital referrals |
| Intelligence % of revenue | 0% | 30-60% at L1-L2 | 20-40% at L1-L2 |
| Capital access % of revenue | 0% | 0% | 20-40% (highest ROI: built once, scales with deal flow) |
| Intelligence infrastructure | — | $120–225/month | $120–225/month |

### Capital Access Revenue Model

**Principle: If they could Google it, it's free. If it's personalized to THEIR deal, it's worth paying for.**

| Source | Free or Paid? | Why? | Per-Deal Revenue | When Active |
|--------|--------------|------|-----------------|-------------|
| Lender intelligence in conversation | **FREE** | Yulia names top lenders for their deal type — powered by public SBA data. Makes Yulia smart, not a paywall. | $0 (builds trust + retention) | Phase G (Month 2) |
| SBA eligibility check (conversational) | **FREE** | Basic "are you eligible?" — anyone can look up SOP 50.10.8 rules | $0 (hooks buyers into platform) | Phase B (Pre-launch) |
| Capital structure advice (conversational) | **FREE** | General guidance on SBA vs conventional vs mezzanine — Googleable | $0 (demonstrates expertise) | Phase A (Launch) |
| **SBA Bankability Report** | **PAID $75** | Full modeling with THEIR numbers: DSCR at their price, equity injection from their specific assets, seller note standby impact. A lender-ready document. | $75 | Phase B (Pre-launch) |
| **Capital Structure Analysis** | **PAID $125** | Personalized capital stack with DSCR sensitivity at 3 rate scenarios, equity injection waterfall, monthly debt service schedule. Can't Google this. | $125 | Phase D (Launch) |
| Lendio/SmartBiz/Guidant referrals | **FREE to user** | User clicks "Apply Now" after Yulia's recommendation. Lender/partner pays us. | $1,000–$5,000 from partner | Phase H (Month 3) |
| Direct SBA lender referrals | **FREE to user** | Pre-qualified lead with full deal package. Lender pays referral fee. | $15,000–$50,000 from lender | Phase I (Month 4) |
| Capital advisory (>$5M deals) | **PAID** success fee | Complex multi-source structuring. 1-2% on capital raised. | $50,000–$200,000 | Phase J (Month 6+) |

**The economics work because the biggest revenue line (lender referral fees) is invisible to the user.** The user gets a great experience — Yulia knows which lenders are active, helps them apply, and the deal funds. The lender pays us for the pre-qualified lead. The user pays only for personalized work product they can't get anywhere else.
...
| Seven-factor scoring model added to Phase B | Referenced 6+ times in public pages, was not built in any phase | Feb 25, 2026 |
| Worker service moved from Phase G to Phase D | Deliverable generation needs background processing, establishes pattern for G | Feb 25, 2026 |
| SBA Bankability Report ($75) added to BUY B1 | Dock tool #4 + buy journey core feature needed explicit deliverable | Feb 25, 2026 |
| Document upload/parsing added to Phase B | Dock tool #1 + agency CIM promise required file processing pipeline | Feb 25, 2026 |
| Intelligence Maturity Model documented | Explicitly notes Phase A-E uses Claude knowledge, Phase G+ adds real data | Feb 25, 2026 |
| "Search for a business" scoped honestly | No listing data source at launch; Yulia helps frame criteria, not search listings | Feb 25, 2026 |
| Deterministic backbone + selective intelligence | 70% structured code + 30% AI reasoning per deliverable (CrewAI pattern) | Feb 25, 2026 |
| SBA rates: static at launch, live at Phase G | System prompt updated monthly pre-G; FRED API replaces manual updates at G | Feb 25, 2026 |
| Capital structure advisory added to Yulia core competency | Every deal needs financing; Claude already knows SBA/seller financing/ROBS mechanics. Zero build cost | Feb 25, 2026 |
| Capital stack decision tree: deterministic, not AI | 5-tier logic (SBA→504→conventional+mezz→structured→alternative) runs as code, not LLM inference | Feb 25, 2026 |
| Capital Structure Analysis ($125) added to BUY B2 | New deliverable: recommended stack, DSCR sensitivity, equity injection waterfall, debt service schedule | Feb 25, 2026 |
| Lender Match Intelligence ($50→$75) added to Phase G | SBA Activity Reports + FDIC API power data-backed lender ranking by deal type + geography | Feb 25, 2026 |
| ~~Lender Match Intelligence as paid deliverable~~ | KILLED — it's public data reformatted. Yulia shares lender intelligence FREE in conversation. Only personalized work product gets charged. | Feb 25, 2026 |
| "If they could Google it, it's free" pricing rule | Lender names, SBA rules, general capital advice = free (makes Yulia smart). SBA Bankability Report, Capital Structure Analysis = paid (personalized to THEIR deal, can't get it elsewhere). Lender referral fees = free to user, paid by lender. | Feb 25, 2026 |
| Lendio Embedded Marketplace for Phase H (not custom build) | White-label, 75+ lenders, one embed. Don't build matching engine — embed theirs | Feb 25, 2026 |
| Guidant Financial + SmartBiz as first affiliates | ROBS referrals + SBA loan referrals with tracking portals. Minimal integration work | Feb 25, 2026 |
| Direct lender partnerships deferred to Phase I | Need 10+ active buyer deals for negotiating leverage. Affiliates first | Feb 25, 2026 |
| Capital matching engine (ML-powered) deferred to Phase J | Needs historical outcome data to train. Decision tree serves until then | Feb 25, 2026 |
| SBA Lender Activity Reports + FDIC + CDFI data added to Phase G | Free government data powers lender intelligence layer. Same pattern as market intelligence | Feb 25, 2026 |
| Buyer financial profile collected conversationally in Phase B | Credit range, liquid assets, retirement funds, home equity, citizenship. Stored in deals table | Feb 25, 2026 |
| GLBA compliance baseline at Phase H | Privacy policy + encryption when collecting/transmitting borrower financial data. SOC 2 Type II target Month 18 | Feb 25, 2026 |
| Referral tracking from day one of partnerships | deal → lender → referral → application → funded → fee. Attribution chain for revenue reporting | Feb 25, 2026 |
| Capital access as 2nd revenue line (20-40% of total) | Every deal needs financing; referral fees $15K-$50K/deal at direct tier. Highest ROI feature once deal flow exists | Feb 25, 2026 |
| No broker license needed at launch (matching platform) | Structure as information/matching, not brokering. Clear disclaimers. CA CFL license proactively when revenue justifies | Feb 25, 2026 |
| Real-time capital stack optimization is the killer feature | Yulia knows full deal context (financials, buyer profile, seller terms); recalculates stack as terms change. No competitor does this | Feb 25, 2026 |
| Capital stack tiers extended to $1B+ | 7-tier decision tree covers SBA through megafund consortium bids. Tiers 1-4 deterministic code, Tiers 5-7 defer to Yulia's conversational intelligence. No deal size ceiling. | Feb 25, 2026 |

---

*BUILD_PLAN_v6 — February 25, 2026 (capital access update)*
*The master document. Contains everything: roadmap, design system, tech stack, pricing, conversion architecture, Yulia's agentic spec, intelligence engine blueprint, capital access strategy, competitive positioning, and unit economics.*
