...
/**
 * Lifecycle record tools (B2.3-2.6) — chat-first flag-flippers that move
 * deals through the gate-readiness machine. Each is a small server-side
 * tool Yulia calls when the user signals an event happened in the real
 * world. None has a UI button; the user just tells Yulia.
 */
...
async function promoteSourcingTargetToDeal(input: Record<string, any>, userId: number): Promise<string> {
  ...
  // Pull the candidate (table name may vary — try the canonical first)
  let candidate: any = null;
  try {
    const rows = await sql`SELECT * FROM sourcing_candidates WHERE id = ${targetId} LIMIT 1`;
    candidate = rows[0];
  } catch {
    // Table missing or alternate name; surface a friendly error
  ...
  // Best-effort field mapping; sourcing_candidates schema may vary
...
async function recordLoiExecuted(input: Record<string, any>, userId: number): Promise<string> {
  ...
  // NOTE: documentLifecycle.executeDocument operates on data_room_documents,
  // not on deliverables, so we don't try to chain the LOI document's
  // lifecycle from here. The deliverables-side equivalent (status='executed')
  // is a separate batch — for B4 → B5 readiness, the deal flag-flip above
  // is what matters.
...
async function closeDeal(input: Record<string, any>, userId: number): Promise<string> {
  ...
  // Optional PMI spawn for BUY closes
