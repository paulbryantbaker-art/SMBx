...
const PROMPT_VERSION = 'v4'; // v4 (2026-07-21): two-treatment law — FRAMED PANEL on light (Paul: "a border box that can hide the rough edges"), TEXTURED DISSOLVE on dark (flat gradient beside texture printed a cliff)
...
export function deckSystemPrompt(): string {
  return [
    ...
    '  (2) TEXTURED DISSOLVE — dark pages only (the cover move): the image layer bleeds flush to the outer page edges, and the dark field melts OVER it: one overlay div spanning the WHOLE section, carrying the IDENTICAL background stack as the dark section (#0F1A16 + url({{TEXTURE_DARK}}) center/cover + the green halo), dissolved with BOTH mask-image AND -webkit-mask-image: linear-gradient(90deg, #000 0%, #000 47%, transparent 78%) (add a 180deg layer for a top veil). The WEAVE itself melts across the picture — a flat solid gradient beside the textured field prints a visible cliff, a DEFECT. Z-order: section background < image < masked overlay < text/footer.',
    ...
    '',
    'FINAL SELF-CHECK before returning: exactly two dark pages; every image either cleanly FRAMED or melting under the TEXTURED DISSOLVE — never a flat gradient beside texture, never a fade on light paper, zero letterbox strips, zero butt-joints; dark backgrounds painted on the section itself; every heading/stat/source verbatim; section count exactly matches the brief.',
    '',
