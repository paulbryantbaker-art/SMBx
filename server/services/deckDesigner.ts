...
/* CARTA (2026-08-15, Paul: "no ledger at all. carta"). MINT_RING survives —
   the ringed headshot is Carta's byline grammar too. GREEN_HALO and
   BLOCK_GLAZE are GONE with the jade block: Carta's band is a flat colour,
   so a halo and a glaze have nothing to sit on. */
import { CARTA, CARTA_TYPE, brandPaletteLines, MINT_RING } from '../../house/tokens.js';
...
export function deckSystemPrompt(): string {
  return [
    ...
    ...brandPaletteLines(),
    `- Type: ${CARTA_TYPE.display} (display serif, weight 550; 600 for card titles) for headlines and pull-lines; ${CARTA_TYPE.sans} for everything else (numerals font-weight 700, tabular figures, letter-spacing -0.02em); ${CARTA_TYPE.mono} ONLY for small labels/sources (13px+ equivalent, letter-spacing 0.16em). Fraunces and Inter are RETIRED — naming either is a reject. Fonts are already loaded, just use the family names.`,
    `- Dark pages are a FLAT NEAR-BLACK BAND: background ${CARTA.dark}, and nothing else. No texture image, no glaze, no halo, no gradient, no bloom. This inverts the previous instruction, which described a jade block built as a three-layer composite and insisted "nothing in this system is near-black" — Carta retired the jade band, the boardroom texture and the whole atmosphere stack on 2026-08-08, and the band is now simply a colour. Do NOT reach for {{TEXTURE_DARK}}: it is a Ledger asset and a page carrying it is a reject. Paint the colour on the SECTION element itself, never only on an inner column, so a sub-pixel gap between children can only ever reveal the band and never a light page beneath. Reading text on it is ${CARTA.darkInk}, sub-text ${CARTA.darkSub}, muted ${CARTA.darkMuted}, hairlines and seams ${CARTA.darkSeam}, label plates ${CARTA.darkPlate}.`,
    '',
    ...
    `  (1) FRAMED PANEL — the ONLY image treatment, on light pages and dark alike: the image sits in a SQUARE editorial frame — border 1px ${CARTA.hair} on light, 1px ${CARTA.darkSeam} on dark, BORDER-RADIUS 0, overflow:hidden, background ${CARTA.white}, the image filling it 100%/100%, and no shadow. Radius is 0 everywhere in Carta except buttons and inputs, so the 18-24px this used to ask for is now a reject — and a translucent shadow is a PDF transparency group, which the renderer-proof law forbids. NEVER fade an image into the page: on paper a fade reads as a printing smudge. The crisp square frame IS the grammar.`,
    `  (2) THERE IS NO SECOND TREATMENT. A full-bleed "textured dissolve" used to be allowed on dark pages — the dark field melting over the image with a mask, so the WEAVE carried across the seam. It depended entirely on the boardroom texture, and Carta has no texture: masking a flat colour over a photo produces a grey smear, not a dissolve. Frame every image. A masked or gradient-faded image edge is a reject.`,
    ...
    '',
    `STRUCTURE LAW: EXACTLY two dark pages — page 1 (the cover) and the final page (the closer). Everything between is white. The cover leads with the hook set large in the display serif (two-tone: the second sentence/beat in mint ${CARTA.mint} on the dark band) and, if the cover has an image, uses the house cover grammar — the FRAMED PANEL ON DARK: the image sits in a square framed card (border 1px ${CARTA.darkSeam}, radius 0, background ${CARTA.white}, overflow hidden) inset ~60px from the top and right page edges and clearing the footer band, filled with object-fit:cover — bright and unfaded against the flat band. The text column sits left, vertically centred. Lay NO glaze over the section: the band is already a flat colour, and stacking a translucent wash on it only makes a slightly different flat colour while adding a transparency group the renderer-proof law forbids. Do NOT melt or fade the cover image. The closer carries the takeaway payoff: a small GREEN mono tag "FOR THE ACQUIRER" (${CARTA.mint} on the band — brass is retired), the takeaway headline in the display serif at ${CARTA.darkInk}, its body, then the byline (ringed headshot + name + title), {{LOGO_WHITE}}, and a mint mono line "FOLLOW FOR THE NEXT READ." Light pages carry a small header lockup ({{LOGO}} + a mono section label) and a footer strip: either a slim band of ${CARTA.dark} ({{LOGO_WHITE}} at 34px + mono page "n / N") or an equally consistent light grammar — but be CONSISTENT across all light pages.`,
    '',
    `CONTENT LAW (zero hallucination — hard rule): use the provided headings, bodies, stats and sources VERBATIM. Never invent, round, extend, or reword a number, source, or claim. You may choose which provided element is the visual hero of a page (e.g., set the number huge in the working sans with a GREEN bar under it — ${CARTA.green}, the house's signature numeral treatment; brass bars are retired — or lead with the heading), but the words themselves are fixed. Every stat page must show its source line (mono, muted).`,
    '',
    `LEGIBILITY: body text ≥24px, sources ≥17px, headlines 40-64px (display serif at 550), hero numerals 110-190px (working sans at 700, tabular). Generous margins (≥76px sides). No element within 40px of another element it does not relate to.`,
    '',
...
export function assembleDeckHtml(raw: string, inp: DesignInputs, fontsHead: string): string | null {
  ...
  <style>@layer pgbase { .pg { background: ${CARTA.bone}; } section.pg:first-of-type, section.pg:last-of-type { background: ${CARTA.dark}; } }
