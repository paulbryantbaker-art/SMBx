...
export function deckSystemPrompt(): string {
  return [
    ...
    `- Dark pages (the JADE BLOCK — nothing in this system is near-black): background ${LEDGER.dark}, then the texture image url({{TEXTURE_DARK}}) (background-size:cover), then a glaze of the base colour OVER the texture — background-image: linear-gradient(${BLOCK_GLAZE}, ${BLOCK_GLAZE}), url({{TEXTURE_DARK}}). The glaze is MANDATORY: the texture is a near-black image and it sits above the colour, so without it the page renders black and the jade never appears at all. Finish with a soft jade halo (radial-gradient, ${GREEN_HALO} fading out). Paint it on the SECTION element itself — never only on an inner column div — so a sub-pixel gap between children can only ever reveal dark, never a light page beneath.`,
    '',
    ...
    '',
    'STRUCTURE LAW: EXACTLY two dark pages — page 1 (the cover) and the final page (the closer). Everything between is light bone. The cover leads with the hook set large in Fraunces (two-tone: the second sentence/beat in mint on dark) and, if the cover has an image, uses the house cover grammar — the FRAMED PANEL ON DARK: the image sits in a crisp framed card (border 1px rgba(242,251,246,0.18), border-radius 24px, background #fff, overflow hidden) inset ~60px from the top and right page edges and clearing the footer band, filled completely with object-fit:cover — bright, unfaded, POPPING against the boardroom texture; the text column sits left on the dark field, vertically centered. Keep the dark field QUIET so it reads as background: lay a further flat glaze over the section (e.g. linear-gradient(180deg, rgba(10,106,76,0.30), rgba(10,106,76,0.48)) above the base stack, under the text) — keep it light, the ground is already glazed — the framed image and the headline carry the page, the texture only whispers. Do NOT melt or fade the cover image into the dark. The closer carries the takeaway payoff: a small brass mono tag "FOR THE ACQUIRER", the takeaway headline in Fraunces ivory, its body, then the byline (ringed headshot + name + title), {{LOGO_WHITE}}, and a mint mono line "FOLLOW FOR THE NEXT READ." Light pages carry a small header lockup ({{LOGO}} + a mono section label) and a footer strip: either a slim dark band (#0A6A4C, {{LOGO_WHITE}} at 34px + mono page "n / N") or an equally consistent light grammar — but be CONSISTENT across all light pages.',
    '',
...
export function assembleDeckHtml(raw: string, inp: DesignInputs, fontsHead: string): string | null {
  ...
  <style>@layer pgbase { .pg { background: ${LEDGER.bone}; } section.pg:first-of-type, section.pg:last-of-type { background: ${LEDGER.dark}; } }
