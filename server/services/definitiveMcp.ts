...
/**
 * Agent-facing decision cards — one per tool.
 *
 * The `description` tells an agent what a tool does; this guide tells it, in
 * STRUCTURED form it can filter on, whether this is the tool it needs right now:
 * when to reach for it, when NOT to (where THE LINE lives at selection time, so
 * the agent self-deflects before calling), what state it needs (`requires` — the
 * field that makes "I was handed one narrow task" work, since it shows which tools
 * run standalone), what it returns, which tools usually come next (so the agent
 * iterates the methodology instead of guessing), and the lifecycle stage it serves.
 *
 * Surfaced on the MCP `tools/list` (`title` + `_meta.guide`) and the server-card.
 * Grounded in each tool's real description / TOOL_SCOPE / THE LINE boundary — not
 * invented. Zero-hallucination applies to tool metadata too.
 */
type DefinitiveToolStage =
  | 'meta' | 'intake' | 'ioi' | 'loi' | 'diligence'
  | 'model' | 'structure' | 'negotiation' | 'close' | 'pmi' | 'sourcing';

type DefinitiveToolRequires =
  | 'none' | 'partial_facts' | 'deal_state_or_facts' | 'deal_id' | 'model_input' | 'package';

interface DefinitiveToolGuide {
  /** Short, legible label — what a human/agent would call this tool. */
  title: string;
  /** One line: what it is for. */
  purpose: string;
  /** Situations where this is the right call. */
  whenToUse: string[];
  /** Wrong-tool / out-of-lane cases (THE LINE at selection time). */
  whenNotToUse: string[];
  /** State precondition — the key signal for narrow-task agents. */
  requires: DefinitiveToolRequires;
  /** The take-back artifact / output it returns. */
  produces: string;
  /** Tools that usually follow — the methodology's next step. */
  typicalNext: DefinitiveMcpToolName[];
  /** Lifecycle stage(s) this tool serves. */
  stage: DefinitiveToolStage[];
  /** THE LINE boundary note, or null for pure read/meta tools. */
  lineBoundary: string | null;
}

const DEFINITIVE_TOOL_GUIDE: Record<DefinitiveMcpToolName, DefinitiveToolGuide> = {
  // ── Orientation & methodology education (call these to get your bearings) ──
  assess_deal_entry: {
    title: 'Assess deal entry point',
    purpose: 'Figure out where you are entering the M&A lifecycle from whatever facts you have, and get the exact next calls.',
    whenToUse: ['You are starting cold or returning to a deal and need the current stage plus what to do next', 'You have only partial facts (even EV-only) and do not know the whole process'],
    whenNotToUse: ['You already know the precise tool to run — call it directly'],
    requires: 'partial_facts',
    produces: 'AgentEntryAssessment: inferred journey/stage, accepted missing inputs, model/document routing, next_suggested_calls',
    typicalNext: ['ingest_deal_payload', 'introspect_capabilities', 'describe_methodology'],
    stage: ['meta', 'intake'],
    lineBoundary: 'Routes software work; never rejects you for incomplete info, and does not advise or decide.',
  },
  introspect_capabilities: {
    title: 'Introspect capabilities for this deal',
    purpose: 'Get a deal-scoped catalog of relevant lifecycle stages, surfaces, take-back artifacts, and matching M-slots.',
    whenToUse: ['You want to know which capabilities and models apply to THIS deal shape before acting', 'You need a compact tool inventory for the matching surface'],
    whenNotToUse: ['You need the full methodology doctrine — use describe_methodology'],
    requires: 'none',
    produces: 'CapabilityCatalog scoped by journey/league/dealType/gates with next_suggested_calls',
    typicalNext: ['assess_deal_entry', 'compose_model_stack', 'get_deal_runbook'],
    stage: ['meta'],
    lineBoundary: null,
  },
  describe_methodology: {
    title: 'Describe the DEFINITIVE methodology',
    purpose: 'Learn the Deal OS doctrine: lifecycle, M101-M223 models, G1-G30 gates, schemas, conformance, and THE LINE.',
    whenToUse: ['You are new to the substrate and need to understand how it works', 'You need the authoritative version pins, gate routing, or THE LINE boundary'],
    whenNotToUse: ['You only need one model-slot detail — use lookup_model_slot'],
    requires: 'none',
    produces: 'Methodology description (overview/lifecycle/models/gates/schemas/line/agent_access sections)',
    typicalNext: ['get_deal_runbook', 'introspect_capabilities', 'lookup_model_slot'],
    stage: ['meta'],
    lineBoundary: null,
  },
  get_deal_runbook: {
    title: 'Get the journey runbook',
    purpose: 'Get the step-by-step buy/sell/raise/PMI runbook so you know how to work the full iterative lifecycle.',
    whenToUse: ['You want the ordered stages, minimum inputs, primary tools, and done-when for a journey'],
    whenNotToUse: ['You need this deal’s current position — use assess_deal_entry or check_completeness'],
    requires: 'none',
    produces: 'Journey runbook(s): stages, minimum inputs, primary tools, work surfaces, done-when, representative model slots',
    typicalNext: ['assess_deal_entry', 'ingest_deal_payload', 'compose_deal_plan'],
    stage: ['meta'],
    lineBoundary: null,
  },
  get_definition_of_done: {
    title: 'Get definitions of done',
    purpose: 'Learn what "done" means at each readiness level (DRL0-DRL4) from intake to PMI.',
    whenToUse: ['You want the gate/readiness criteria and which tools unlock the next level', 'An overlay gate (G28/G29/G30) was triggered and you need its boundary'],
    whenNotToUse: ['You want to score THIS deal — use check_completeness'],
    requires: 'none',
    produces: 'Versioned definitions of done with per-level done-when and next recommended tools',
    typicalNext: ['check_completeness', 'compose_deal_plan'],
    stage: ['meta'],
    lineBoundary: null,
  },
  lookup_model_slot: {
    title: 'Look up a model slot (M101-M223)',
    purpose: 'Inspect one DEFINITIVE model slot: gates, deal types, authorities, runtime mapping, readiness, and boundary.',
    whenToUse: ['You want to know whether a specific mechanic is executable now and what it needs', 'You are deciding which model to run'],
    whenNotToUse: ['You want to actually compute it — use execute_model'],
    requires: 'none',
    produces: 'Model-slot detail (or compact catalog) with readiness, runtime model id, citations, next_suggested_calls',
    typicalNext: ['execute_model', 'compose_model_stack', 'lookup_citation'],
    stage: ['meta', 'model'],
    lineBoundary: 'Tells you the slot’s boundary: executable vs professional-handoff vs research-only.',
  },
  lookup_citation: {
    title: 'Resolve a citation / authority',
    purpose: 'Resolve a claim or citation tag against the Authority Register and V19 citation registry.',
    whenToUse: ['You need the source behind a number or claim (zero-hallucination support)', 'You are validating a model output’s authority anchors'],
    whenNotToUse: ['You need market-data values — use fetch_market_data'],
    requires: 'none',
    produces: 'Resolved authority/citation records with source and jurisdiction',
    typicalNext: ['execute_model', 'describe_methodology'],
    stage: ['meta'],
    lineBoundary: null,
  },
  fetch_market_data: {
    title: 'Fetch market / regulatory data',
    purpose: 'Get timestamped market/regulatory series or a market-multiple packet for valuation/LBO/comps.',
    whenToUse: ['A model needs entry/exit/comparable multiples or a rate (SOFR, prime, HSR thresholds)', 'You want source and freshness metadata for an assumption'],
    whenNotToUse: ['You want the model run itself — pass the packet to execute_model'],
    requires: 'none',
    produces: 'Market series value(s) or a MarketMultiplePacket with provenance and freshness',
    typicalNext: ['execute_model', 'run_model_iteration'],
    stage: ['meta', 'model'],
    lineBoundary: null,
  },
  estimate_deal_cost: {
    title: 'Estimate workspace cost',
    purpose: 'Estimate the monthly workspace level and pass-through posture for agentic deal work.',
    whenToUse: ['You want to size the subscription/credits for expected model runs, API calls, Studio exports, and seats'],
    whenNotToUse: ['You expect per-deal or success-based pricing — that does not exist here'],
    requires: 'none',
    produces: 'Monthly workspace level and a THE LINE-safe pass-through estimate (no wallet, no success fee)',
    typicalNext: ['describe_methodology'],
    stage: ['meta'],
    lineBoundary: 'Monthly subscription only; no fee tied to deal value or outcome.',
  },
  validate_conformance: {
    title: 'Validate conformance status',
    purpose: 'Read the DB-free DEFINITIVE conformance status, case count, and validation command.',
    whenToUse: ['You want to confirm the substrate’s conformance baseline before relying on it'],
    whenNotToUse: ['You want to run a model — use execute_model'],
    requires: 'none',
    produces: 'Conformance status, case count, categories, validation command',
    typicalNext: ['describe_methodology'],
    stage: ['meta'],
    lineBoundary: null,
  },

  // ── Deal state: create / read / evolve ──
  ingest_deal_payload: {
    title: 'Ingest deal facts → DealState',
    purpose: 'Hand over whatever deal facts you have and get a classified, content-addressed DealState back (never rejected).',
    whenToUse: ['You are starting a deal or have new facts and want a durable DealState to work from'],
    whenNotToUse: ['You already have a DealState — use update_deal_payload'],
    requires: 'partial_facts',
    produces: 'DealState + MissingInputContract + completeness score + next_suggested_calls',
    typicalNext: ['check_completeness', 'compose_deal_plan', 'compose_model_stack'],
    stage: ['intake'],
    lineBoundary: 'Classifies and routes; accepts partial info instead of rejecting.',
  },
  update_deal_payload: {
    title: 'Update DealState with a patch',
    purpose: 'Merge new facts into an existing DealState and recompute classification, completeness, and next calls.',
    whenToUse: ['You have new facts, documents, or answers to fold into a live deal'],
    whenNotToUse: ['You are creating the deal for the first time — use ingest_deal_payload'],
    requires: 'deal_state_or_facts',
    produces: 'Updated DealState + recomputed MissingInputContract + completeness + next_suggested_calls',
    typicalNext: ['check_completeness', 'compose_deal_plan'],
    stage: ['intake', 'diligence'],
    lineBoundary: null,
  },
  get_deal_state: {
    title: 'Read a saved DealState',
    purpose: 'Read the latest persisted DealState by dealId/conversationId/stateCid so you can resume without scraping the app.',
    whenToUse: ['You are returning to a deal and need its current persisted state'],
    whenNotToUse: ['You have no prior deal — use assess_deal_entry or ingest_deal_payload'],
    requires: 'deal_id',
    produces: 'The latest DealState snapshot',
    typicalNext: ['resume_deal', 'compose_deal_plan', 'check_completeness'],
    stage: ['meta'],
    lineBoundary: null,
  },
  check_completeness: {
    title: 'Score readiness vs definitions of done',
    purpose: 'Score a DealState/payload against the lifecycle definitions of done and name the next gate.',
    whenToUse: ['You want the readiness level, blockers, and the next gate to clear'],
    whenNotToUse: ['You want the generic DoD doctrine — use get_definition_of_done'],
    requires: 'deal_state_or_facts',
    produces: 'DRL level, missing inputs, blockers, next gate, next_suggested_calls',
    typicalNext: ['update_deal_payload', 'compose_model_stack', 'compose_deal_plan'],
    stage: ['meta'],
    lineBoundary: null,
  },
  compose_deal_plan: {
    title: 'Compose the deal plan',
    purpose: 'Get a portable plan of the full lifecycle with current stage, blocked stages, surfaces, and next actions.',
    whenToUse: ['You want the whole roadmap for this deal and what is blocking each stage'],
    whenNotToUse: ['You only want a readiness score — use check_completeness'],
    requires: 'deal_state_or_facts',
    produces: 'DealPlan (stages, current/blocked, work surfaces, next actions)',
    typicalNext: ['compose_deal_package', 'compose_model_stack'],
    stage: ['meta'],
    lineBoundary: null,
  },
  diff_deal_state: {
    title: 'Diff two DealState snapshots',
    purpose: 'Compare two snapshots and see what changed: paths, completeness delta, resolved/new missing inputs, gate changes.',
    whenToUse: ['You want to understand what a set of new facts changed', 'You are reconciling parallel or branch work'],
    whenNotToUse: ['You only have one state — there is nothing to diff'],
    requires: 'deal_state_or_facts',
    produces: 'DealStateDiff (changed paths, completeness delta, overlay-gate changes)',
    typicalNext: ['compose_deal_plan', 'check_completeness'],
    stage: ['meta'],
    lineBoundary: null,
  },
  clone_deal_state: {
    title: 'Clone DealState for scenarios',
    purpose: 'Branch a DealState into a new revision for scenario work or parallel agents, preserving lineage.',
    whenToUse: ['You want to explore an alternative set of assumptions without disturbing the base deal'],
    whenNotToUse: ['You want to update the base deal in place — use update_deal_payload'],
    requires: 'deal_state_or_facts',
    produces: 'Cloned DealState revision (parent CID preserved) with the same contracts',
    typicalNext: ['generate_permutations', 'run_model_iteration'],
    stage: ['meta', 'structure'],
    lineBoundary: null,
  },
  resume_deal: {
    title: 'Resume iterative deal work',
    purpose: 'Pick up a deal and get stage, plan, package, completeness, missing inputs, and next calls in one shot.',
    whenToUse: ['You are re-entering a deal and want everything needed to continue the loop'],
    whenNotToUse: ['You are creating a deal — use ingest_deal_payload'],
    requires: 'deal_state_or_facts',
    produces: 'Current stage + DealPlan + DealPackage + completeness + next_suggested_calls + take-back artifacts',
    typicalNext: ['check_completeness', 'compose_model_stack', 'compose_deal_package'],
    stage: ['meta'],
    lineBoundary: null,
  },
  compose_lifecycle_trace: {
    title: 'Compose the lifecycle trace',
    purpose: 'Build a portable history of the deal: stage trace, source/artifact refs, blockers, and loop contract.',
    whenToUse: ['You (or a human) need to understand how the deal got to where it is'],
    whenNotToUse: ['You want the forward plan — use compose_deal_plan'],
    requires: 'deal_state_or_facts',
    produces: 'LifecycleTrace (current stage, stage trace, refs, blockers, next_suggested_calls)',
    typicalNext: ['compose_deal_plan', 'resume_deal'],
    stage: ['meta'],
    lineBoundary: null,
  },

  // ── Portable package: take back / verify / finalize ──
  compose_deal_package: {
    title: 'Compose a portable deal package',
    purpose: 'Bundle the deal’s classification, completeness, plan, sources, and next calls into a portable take-back package.',
    whenToUse: ['You are pausing or handing off and want a self-contained snapshot to carry back'],
    whenNotToUse: ['You want to verify an existing package — use verify_package'],
    requires: 'deal_state_or_facts',
    produces: 'DealPackage (classification, completeness, plan, source index, deferrals, next calls)',
    typicalNext: ['verify_package', 'finalize_deal_package'],
    stage: ['meta'],
    lineBoundary: null,
  },
  verify_package: {
    title: 'Verify a deal package',
    purpose: 'Recompute and check a package’s integrity before you rely on it or take it back.',
    whenToUse: ['You received a package and want to confirm its CID/state identity and take-back artifacts'],
    whenNotToUse: ['You want to stamp/sign it — use finalize_deal_package'],
    requires: 'package',
    produces: 'PackageVerification.v0.1 (no legal/tax/fairness determination)',
    typicalNext: ['finalize_deal_package', 'reopen_deal_package'],
    stage: ['meta'],
    lineBoundary: 'Verifies software identity only; makes no legal/tax/fairness/solvency/closing determination.',
  },
  finalize_deal_package: {
    title: 'Finalize / sign a deal package',
    purpose: 'Stamp a verified package into signed take-back artifacts (audit packet, signed manifest, attestation, proof).',
    whenToUse: ['A human has approved and you want an immutable, auditable package manifest'],
    whenNotToUse: ['You have not verified it yet — call verify_package first', 'You think this authorizes closing — it does not'],
    requires: 'package',
    produces: 'PackageVerification + AuditPacket + SignedManifest + Attestation + MerkleInclusionProof',
    typicalNext: ['compose_close_readiness', 'disclose_subset'],
    stage: ['close'],
    lineBoundary: 'Stamps the software manifest only; does not authorize closing, move funds, negotiate, or advise. Needs human approval.',
  },
  reopen_deal_package: {
    title: 'Reopen a deal package',
    purpose: 'Turn a package back into a live DealState revision when new facts arrive, preserving lineage.',
    whenToUse: ['New facts, documents, model outputs, or diligence answers came in after packaging'],
    whenNotToUse: ['Nothing changed — keep using the package'],
    requires: 'package',
    produces: 'A new content-addressed DealState revision with package + prior-state lineage',
    typicalNext: ['update_deal_payload', 'check_completeness'],
    stage: ['meta'],
    lineBoundary: null,
  },

  // ── Structure permutations (compare, never recommend) ──
  generate_permutations: {
    title: 'Generate structure permutations',
    purpose: 'Produce deterministic deal-structure options as a Pareto frontier under a stated objective.',
    whenToUse: ['You want to compare possible deal vehicles/structures objectively'],
    whenNotToUse: ['You want the single best under weights — use compute_best_vehicle', 'You want a recommendation — the substrate does not recommend'],
    requires: 'deal_state_or_facts',
    produces: 'ParetoFrontier of StructurePermutations (no recommendation)',
    typicalNext: ['score_permutation', 'compute_best_vehicle', 'expand_permutations'],
    stage: ['structure'],
    lineBoundary: 'Compares structures; does not recommend a transaction structure.',
  },
  score_permutation: {
    title: 'Score one structure',
    purpose: 'Deterministically score a single structure under a stated objective preference.',
    whenToUse: ['You want the computed score and model output for one specific structure'],
    whenNotToUse: ['You want the full set — use generate_permutations'],
    requires: 'deal_state_or_facts',
    produces: 'StructurePermutation + ModelOutput',
    typicalNext: ['compute_best_vehicle', 'generate_permutations'],
    stage: ['structure'],
    lineBoundary: 'Computes a score; the user and advisors decide whether to rely on it.',
  },
  set_objective_preference: {
    title: 'Set objective preference',
    purpose: 'Record the caller’s objective (e.g. seller_cash, certainty, speed) and return the matching frontier.',
    whenToUse: ['You want to bias the structure comparison toward a stated objective'],
    whenNotToUse: ['You want a recommendation — this only records preference for computation'],
    requires: 'deal_state_or_facts',
    produces: 'ParetoFrontier under the recorded preference',
    typicalNext: ['compute_best_vehicle', 'generate_permutations'],
    stage: ['structure'],
    lineBoundary: 'Records preference for computation only; does not recommend.',
  },
  compute_best_vehicle: {
    title: 'Compute best vehicle (under weights)',
    purpose: 'Return the highest-scoring structure point under the caller’s weights, with unresolved handoffs.',
    whenToUse: ['You want the computed-best structure given explicit weights'],
    whenNotToUse: ['You expect advice — this is computed-best under YOUR weights, not a recommendation'],
    requires: 'deal_state_or_facts',
    produces: 'BestVehicleBlock + ParetoFrontier + unresolved handoffs',
    typicalNext: ['prepare_loi_packet', 'compose_document_draft'],
    stage: ['structure'],
    lineBoundary: 'Computed-best under supplied weights, not advice or a recommendation.',
  },
  expand_permutations: {
    title: 'Expand the permutation set',
    purpose: 'Broaden the structure search with overlay and second-order variants.',
    whenToUse: ['You need a wider vehicle search before packaging'],
    whenNotToUse: ['The base frontier already covers your options'],
    requires: 'deal_state_or_facts',
    produces: 'Updated ParetoFrontier with expanded variants',
    typicalNext: ['compute_best_vehicle', 'score_permutation'],
    stage: ['structure'],
    lineBoundary: null,
  },

  // ── Lifecycle artifacts (IOI → PMI) ──
  prepare_ioi_packet: {
    title: 'Prepare an IOI packet',
    purpose: 'Organize known facts, preliminary economics, source gaps, and model dependencies for pre-LOI indication.',
    whenToUse: ['You are preparing an indication of interest and want a structured packet'],
    whenNotToUse: ['You want to make or transmit an offer, or give a valuation opinion — out of lane'],
    requires: 'deal_state_or_facts',
    produces: 'IOIPacket (facts, preliminary economics, source gaps, next_suggested_calls)',
    typicalNext: ['prepare_loi_packet', 'compose_model_stack', 'compose_document_draft'],
    stage: ['ioi'],
    lineBoundary: 'Organizes; does not make an offer, give a valuation opinion, recommend economics, or transmit externally.',
  },
  prepare_loi_packet: {
    title: 'Prepare an LOI packet',
    purpose: 'Organize structure, economic terms, conditions, source gaps, and handoffs for LOI architecture.',
    whenToUse: ['You are scaffolding an LOI and want structure/terms/conditions organized'],
    whenNotToUse: ['You want clause language, a binding offer, a legal/tax opinion, or to negotiate — out of lane'],
    requires: 'deal_state_or_facts',
    produces: 'LOIPacket (structure, terms, conditions, gaps, handoffs, next_suggested_calls)',
    typicalNext: ['compose_document_draft', 'prepare_diligence_request', 'defer_to_counsel'],
    stage: ['loi'],
    lineBoundary: 'Organizes; does not draft clauses, make a binding offer, give legal/tax opinions, negotiate, or transmit.',
  },
  compose_data_room_index: {
    title: 'Compose a data-room index',
    purpose: 'Group source files into diligence buckets, flag source gaps, and surface next calls.',
    whenToUse: ['You have documents/sources and want them organized into a diligence index'],
    whenNotToUse: ['You want to send diligence asks — use prepare_diligence_request'],
    requires: 'deal_state_or_facts',
    produces: 'DataRoomIndex (buckets, source gaps, next_suggested_calls)',
    typicalNext: ['prepare_diligence_request', 'disclose_subset'],
    stage: ['diligence'],
    lineBoundary: null,
  },
  prepare_diligence_request: {
    title: 'Prepare a diligence request',
    purpose: 'Organize source-backed diligence asks by bucket with gaps, model dependencies, and handoffs.',
    whenToUse: ['You want a structured list of what to request, by diligence bucket'],
    whenNotToUse: ['You want to transmit it or make legal demands — out of lane'],
    requires: 'deal_state_or_facts',
    produces: 'DiligenceRequest (asks by bucket, gaps, handoffs, next_suggested_calls)',
    typicalNext: ['compose_data_room_index', 'disclose_subset', 'update_deal_payload'],
    stage: ['diligence'],
    lineBoundary: 'Organizes asks; does not transmit, make legal demands, or decide what you should send.',
  },
  disclose_subset: {
    title: 'Compose a disclosure subset',
    purpose: 'Build a selective-disclosure manifest and proof of a data-room subset for take-back.',
    whenToUse: ['You need to scope exactly which sources to expose for a purpose/audience'],
    whenNotToUse: ['You want to actually share externally — that needs a separate A5 approval (share_document)'],
    requires: 'deal_state_or_facts',
    produces: 'DisclosureSubset manifest + proof (does not transmit)',
    typicalNext: ['share_document', 'prepare_diligence_request'],
    stage: ['diligence'],
    lineBoundary: 'Composes a manifest only; external sharing requires a separate A5 approval action.',
  },
  compose_document_draft: {
    title: 'Compose a Studio document draft',
    purpose: 'Scaffold a Studio document preserving source requirements, model dependencies, and THE LINE boundaries.',
    whenToUse: ['You want a structured draft scaffold (deal brief, IOI, LOI outline, IC memo, etc.) to iterate in Studio'],
    whenNotToUse: ['You want a queued/exported deliverable — use generate_output_doc'],
    requires: 'deal_state_or_facts',
    produces: 'DocumentDraft scaffold (source reqs, model deps, next_suggested_calls)',
    typicalNext: ['generate_output_doc', 'execute_model'],
    stage: ['ioi', 'loi', 'diligence', 'negotiation'],
    lineBoundary: 'Scaffolds; preserves source/model requirements and does not export externally.',
  },
  prepare_negotiation_brief: {
    title: 'Prepare a negotiation brief',
    purpose: 'Organize open terms, source gaps, and model-backed range status for negotiation prep.',
    whenToUse: ['You want the open terms and their model-backed ranges organized'],
    whenNotToUse: ['You want to negotiate, recommend concessions, or draft binding language — out of lane'],
    requires: 'deal_state_or_facts',
    produces: 'NegotiationBrief (open terms, gaps, range status, handoffs, next_suggested_calls)',
    typicalNext: ['compose_document_draft', 'run_model_iteration'],
    stage: ['negotiation'],
    lineBoundary: 'Organizes; does not negotiate, recommend concessions, draft binding language, or transmit.',
  },
  compose_close_readiness: {
    title: 'Compose close readiness',
    purpose: 'Show closing readiness checks, blockers, model dependencies, funds-flow status, and approval handoffs.',
    whenToUse: ['You are orchestrating late-stage closing and want a readiness picture'],
    whenNotToUse: ['You want to authorize closing or move money — out of lane'],
    requires: 'deal_state_or_facts',
    produces: 'CloseReadiness packet (checks, blockers, funds-flow status, approval handoffs)',
    typicalNext: ['generate_funds_flow', 'close_deal', 'finalize_deal_package'],
    stage: ['close'],
    lineBoundary: 'Shows readiness; does not authorize closing, move money, issue wires, act as escrow, or give legal/tax opinions.',
  },
  generate_funds_flow: {
    title: 'Generate a funds-flow',
    purpose: 'Build closing arithmetic: sources, uses, adjustments, reconciliation, gaps, and handoffs.',
    whenToUse: ['You need the closing funds-flow scaffold reconciled'],
    whenNotToUse: ['You want to move money or issue wire instructions — out of lane'],
    requires: 'deal_state_or_facts',
    produces: 'FundsFlow (sources, uses, adjustments, reconciliation, next_suggested_calls)',
    typicalNext: ['compose_close_readiness', 'close_deal'],
    stage: ['close'],
    lineBoundary: 'Arithmetic only; does not move money, issue wires, act as escrow, or give legal/tax opinions.',
  },
  compose_pmi_plan: {
    title: 'Compose a PMI plan',
    purpose: 'Organize Day 0 controls, stabilization, assessment, optimization, and risk tracking after close.',
    whenToUse: ['You are planning post-merger integration after a close'],
    whenNotToUse: ['You want operating decisions or employment/legal/tax advice — out of lane'],
    requires: 'deal_state_or_facts',
    produces: 'PMIPlan (Day 0 through optimization, risks, next_suggested_calls)',
    typicalNext: ['compose_document_draft', 'execute_model'],
    stage: ['pmi'],
    lineBoundary: 'Organizes; does not make operating decisions or give employment/legal/tax advice.',
  },

  // ── Models: stack / execute / iterate / read ──
  compose_model_stack: {
    title: 'Compose the model stack',
    purpose: 'Compose and persist the canonical model stack for a deal, with G28/G29/G30 overlay routing.',
    whenToUse: ['You want the applicable M-slots (with readiness and surfaces) assembled for a saved deal'],
    whenNotToUse: ['You have no saved dealId — ingest first', 'You want to run one model now — use execute_model'],
    requires: 'deal_id',
    produces: 'Applicable M101-M223 mechanics with readiness, tool surfaces, and THE LINE boundaries',
    typicalNext: ['execute_model', 'run_model_iteration', 'generate_output_doc'],
    stage: ['model'],
    lineBoundary: 'Unimplemented catalog models surface as planning/readiness, not executable.',
  },
  execute_model: {
    title: 'Execute a model (one-shot)',
    purpose: 'Run one deterministic V19/DEFINITIVE model and get an audited output with citations. No deal required.',
    whenToUse: ['You have the inputs and want a single computed result', 'You were handed a narrow modeling task'],
    whenNotToUse: ['You want a fairness opinion or valuation determination — out of lane', 'You want versioned iteration — use run_model_iteration'],
    requires: 'model_input',
    produces: 'ModelOutput: value(s), assumptions, citations, missing inputs, output hash, audit payload',
    typicalNext: ['run_model_iteration', 'generate_output_doc', 'lookup_citation'],
    stage: ['model'],
    lineBoundary: 'Computes; does not value for fairness, advise, or recommend.',
  },
  run_model_iteration: {
    title: 'Run / iterate a model (persisted)',
    purpose: 'Run or rerun a model with overrides, persisting execution lineage and parent output hash.',
    whenToUse: ['You want versioned, iterative modeling tied to a deal (not one-and-done)'],
    whenNotToUse: ['You just need a quick standalone calc — use execute_model'],
    requires: 'model_input',
    produces: 'Persisted execution (lineage, parent hash, citations, output hash, next_suggested_calls)',
    typicalNext: ['list_model_executions', 'generate_output_doc'],
    stage: ['model'],
    lineBoundary: 'Computes; does not advise or recommend.',
  },
  list_model_executions: {
    title: 'List model executions',
    purpose: 'See what has already been modeled for a deal, with freshness/rerun status against current assumptions.',
    whenToUse: ['You want prior runs and whether they are current before rerunning or generating a doc'],
    whenNotToUse: ['You have not modeled anything yet — use execute_model'],
    requires: 'deal_id',
    produces: 'Execution list (output hashes, lineage, freshness/rerun status, next_suggested_calls)',
    typicalNext: ['run_model_iteration', 'generate_output_doc'],
    stage: ['model'],
    lineBoundary: null,
  },
  generate_output_doc: {
    title: 'Generate an output document',
    purpose: 'Queue a deal deliverable (term sheet, LOI, IC memo, CIM, funds-flow, etc.) by friendly name from deal state and model lineage.',
    whenToUse: ['You want a real deliverable produced from the deal’s current state and models'],
    whenNotToUse: ['You only want a scaffold to iterate — use compose_document_draft', 'Models are stale and you set requireFreshModels — refresh first'],
    requires: 'deal_id',
    produces: 'A Doc/Studio action + model-dependency context + THE LINE boundaries',
    typicalNext: ['list_model_executions', 'run_model_iteration'],
    stage: ['ioi', 'loi', 'diligence', 'negotiation', 'close'],
    lineBoundary: 'Produces software documents only; not legal/tax/fairness opinions.',
  },

  // ── Authority, market, counsel, corpus ──
  defer_to_counsel: {
    title: 'Defer to counsel (THE LINE)',
    purpose: 'Create a structured deferral packet for legal/tax/regulatory/professional-review questions.',
    whenToUse: ['A question needs a qualified professional opinion (legal/tax/securities/regulatory)', 'You hit THE LINE and must hand off'],
    whenNotToUse: ['The question is a software/data computation you can run instead'],
    requires: 'none',
    produces: 'A THE LINE deferral packet routing the issue to qualified review',
    typicalNext: ['find_providers', 'update_deal_payload'],
    stage: ['meta'],
    lineBoundary: 'This IS the handoff: the substrate routes, qualified professionals opine.',
  },
  scan_market: {
    title: 'Scan the market',
    purpose: 'Discover market signals — buyers, sellers, lenders, advisors, comparable transactions.',
    whenToUse: ['You want substrate-side market intelligence for a scope and criteria'],
    whenNotToUse: ['You expect paid introductions — this is never a paid-matching surface'],
    requires: 'none',
    produces: 'Market signals for the requested scope',
    typicalNext: ['find_providers', 'ingest_deal_payload'],
    stage: ['sourcing'],
    lineBoundary: 'Intelligence only; never a paid-matching surface.',
  },
  find_providers: {
    title: 'Find service providers',
    purpose: 'Surface a directory of counsel/CPA/lender/broker/advisor options with disclosures.',
    whenToUse: ['You need professional options for a category and criteria'],
    whenNotToUse: ['You expect compensated referrals — there are none'],
    requires: 'none',
    produces: 'Provider directory with disclosures (never compensated referrals)',
    typicalNext: ['defer_to_counsel'],
    stage: ['meta'],
    lineBoundary: 'Directory with disclosures; never compensated referrals or paid matching.',
  },
  record_corpus_observation: {
    title: 'Record an anonymized benchmark',
    purpose: 'Record a structured, anonymized deal-term observation — only with an active data-rights grant.',
    whenToUse: ['You want to contribute an anonymized benchmark (e.g. NWC peg, earnout) to the corpus'],
    whenNotToUse: ['No data-rights grant exists', 'You would include raw text or party identifiers — not allowed'],
    requires: 'none',
    produces: 'A stored anonymized observation (raw text and identifiers stripped)',
    typicalNext: [],
    stage: ['meta'],
    lineBoundary: 'Only with an active data-rights grant; identifiers stripped.',
  },

  // ── Write / approval-gated actions ──
  close_deal: {
    title: 'Mark deal closed (approval-gated)',
    purpose: 'Mark a transaction closed — only after explicit human approval.',
    whenToUse: ['A human has explicitly approved closing and you are recording it (confirmed=true)'],
    whenNotToUse: ['No explicit human approval', 'You think this moves money or signs anything — it does not'],
    requires: 'deal_id',
    produces: 'A closed-deal record (and optional PMI workspace) once approved',
    typicalNext: ['compose_pmi_plan'],
    stage: ['close'],
    lineBoundary: 'Requires explicit human approval; proves THE LINE approval envelope. Destructive/irreversible.',
  },
  record_loi_executed: {
    title: 'Record LOI executed (approval-gated)',
    purpose: 'Record that an LOI was countersigned by both parties.',
    whenToUse: ['Both parties countersigned and a human approved recording it'],
    whenNotToUse: ['Not yet countersigned', 'No human approval (A6 permission)'],
    requires: 'deal_id',
    produces: 'An immutable LOI-executed record',
    typicalNext: ['prepare_diligence_request', 'compose_model_stack'],
    stage: ['loi', 'negotiation'],
    lineBoundary: 'Requires human approval (A6_IMMUTABLE_OR_CLOSE).',
  },
  share_document: {
    title: 'Share a document externally (approval-gated)',
    purpose: 'Share a deal artifact with an external party via Files / Data Room.',
    whenToUse: ['A human approved external disclosure and you are performing the share (A5)'],
    whenNotToUse: ['No A5 external-disclosure approval', 'You only need to scope what would be shared — use disclose_subset'],
    requires: 'deal_id',
    produces: 'An external-disclosure action on the named document',
    typicalNext: [],
    stage: ['diligence'],
    lineBoundary: 'External-disclosure action; requires A5_EXTERNAL_DISCLOSURE approval.',
  },
  update_tax_position: {
    title: 'Record tax position (counsel-gated)',
    purpose: 'Organize transaction tax-position facts — only after qualified tax/counsel review clearance.',
    whenToUse: ['Qualified tax/counsel review has cleared and you are recording the facts'],
    whenNotToUse: ['No tax/counsel clearance — use defer_to_counsel first', 'You want a tax opinion — out of lane'],
    requires: 'deal_id',
    produces: 'A recorded tax-position fact set',
    typicalNext: ['compose_close_readiness'],
    stage: ['diligence', 'close'],
    lineBoundary: 'Requires qualified tax/counsel review clearance; the substrate records, professionals opine.',
  },
  update_firm_memory: {
    title: 'Update firm memory (enterprise)',
    purpose: 'Update firm-level shared memory (templates, preferences, deal patterns).',
    whenToUse: ['You are persisting a firm-level template/preference under enterprise scope'],
    whenNotToUse: ['You lack enterprise scope', 'It is deal-specific — keep it in DealState'],
    requires: 'none',
    produces: 'An updated firm-level memory entry',
    typicalNext: [],
    stage: ['meta'],
    lineBoundary: 'Requires enterprise scope.',
  },
  query_admin_data: {
    title: 'Query admin data (enterprise)',
    purpose: 'Privileged administrative query surface, governed by enterprise scope.',
    whenToUse: ['You have governed enterprise scope and need an administrative query'],
    whenNotToUse: ['You lack enterprise scope'],
    requires: 'none',
    produces: 'An administrative query result (governed)',
    typicalNext: [],
    stage: ['meta'],
    lineBoundary: 'Requires governed enterprise scope.',
  },
};
...
export function listDefinitiveMcpTools() {
  return {
    ...
    tools: DEFINITIVE_MCP_TOOLS.map(name => {
      ...
      const guide = DEFINITIVE_TOOL_GUIDE[name];
      return {
        name,
        title: guide.title,
