    ...
    if (assistantText) {
      ...
      if (deal) {
        ...
        try {
          ...
          if (gateResult) {
            // UX-05: enqueue the completion deliverable for the just-completed
            // gate so the user gets an artifact for their progress. The helper
            // inserts a pending row immediately and generates off-thread, so
            // we have a deliverableId to send in the SSE without blocking.
            // Re-fetch the deal so the generator sees the latest field updates
            // applied above.
