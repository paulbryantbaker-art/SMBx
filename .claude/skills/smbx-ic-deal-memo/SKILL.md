...
4. **Returns and structure.** The `smbx-deal-scenarios` output: three cases side by side, the bid ceiling and its determinants, DSCR headroom, and how the contested-earnings gap is structured. State what must go right to hit base case, and what can go wrong without breaching 1.25. Name the bear's KIND — return risk (misses the threshold) or ruin risk (coverage fails, the personal guarantee is called) — and for ruin-shaped bears, the survival runway in months. A mediocre return is recoverable; a called PG is not, and the memo must not let the two read alike.
