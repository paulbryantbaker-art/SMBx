...
export default function InlineArtifact({ artifact, deal }: Props) {
  ...
  if (/cim/.test(slug)) {
    return frame(
      <div>
        <div style={{ fontFamily: "'Figtree', system-ui, sans-serif", fontWeight: 800, fontSize: 20, letterSpacing: '-0.02em', color: 'var(--text-primary)' }}>
          {deal.name} · CIM draft
        </div>
  ...
  if (/loi/.test(slug) && deal.askingPriceLabel) {
    return frame(
      <div>
        <div style={{ fontFamily: "'Figtree', system-ui, sans-serif", fontWeight: 800, fontSize: 22, letterSpacing: '-0.02em', color: 'var(--text-primary)' }}>
          {deal.askingPriceLabel}
          <span style={{ fontSize: 11, color: 'var(--text-muted)', marginLeft: 8, fontWeight: 600, fontFamily: "'Figtree', system-ui, sans-serif", letterSpacing: '0.04em' }}>
            REC
          </span>
        </div>
        <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4, lineHeight: 1.4 }}>
...
function HeadlineStat({ label, value }: { label: string; value: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', padding: '3px 0' }}>
      ...
      <span style={{ fontFamily: "'Figtree', system-ui, sans-serif", fontSize: 15, fontWeight: 800, letterSpacing: '-0.01em', color: 'var(--text-primary)' }}>
        {value}
      </span>
    </div>
  );
