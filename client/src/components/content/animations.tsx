...
export function BeforeAfterSlider({
  ...
  return (
    ...
    >
      ...
      <span style={{ position: 'absolute', top: 12, right: 12, fontSize: 11, fontWeight: 700, color: '#fff', background: '#b0004a', padding: '3px 10px', borderRadius: 100, letterSpacing: '0.06em', textTransform: 'uppercase', zIndex: 3 }}>{afterLabel}</span>
    </div>
  );
}
  ...
  return (
    <div className={className} style={{ background: '#FFFFFF', borderRadius: 16, border: '1px solid rgba(0,0,0,0.06)', padding: '28px 32px' }}>
      ...
      <div style={{ fontSize: '14px', fontVariantNumeric: 'tabular-nums' }}>
            ...
            >
              <span className="flex items-center gap-2">
                <span style={{
                  ...
                  height: 20,
                  borderRadius: 6,
                  border: item.enabled ? '2px solid #b0004a' : '2px solid rgba(0,0,0,0.15)',
                  background: item.enabled ? '#b0004a' : 'transparent',
                  transition: 'all 0.2s',
                  flexShrink: 0,
                }}>
                  {item.enabled && (
              ...
              <span style={{ color: item.enabled ? '#b0004a' : 'rgba(0,0,0,0.25)', fontWeight: 600, flexShrink: 0, marginLeft: 16 }}>
        ...
        >
          <div className="flex justify-between" style={{ fontSize: '16px', fontWeight: 600, color: '#b0004a' }}>
          ...
          <div className="flex justify-between mt-2" style={{ fontSize: '16px', fontWeight: 600, color: '#b0004a' }}>
  ...
  return (
    <div ref={ref} className={className} style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
      {stats.map((s, i) => (
        ...
        >
          <div style={{ fontSize: '28px', fontWeight: 700, color: '#b0004a', lineHeight: 1.1 }}>
...
export function AnimatedTimeline({
  ...
  return (
    <div ref={ref} className={className} style={{ position: 'relative' }}>
      ...
      {/* Animated fill */}
      <motion.div style={{ position: 'absolute', left: 4, top: 0, bottom: 0, width: 2, background: '#b0004a', transformOrigin: 'top', scaleY }} />
      {children}
    </div>
  );
}
      ...
      {items.map((item, i) => (
        <ScrollReveal key={item.title} delay={i * 0.06}>
          <div style={{
            ...
            borderRadius: 20,
            border: '1px solid rgba(0,0,0,0.04)',
            borderLeft: highlight ? '3px solid #b0004a' : undefined,
            padding: '24px 28px',
            height: '100%',
          }}>
            <div className="flex items-start justify-between gap-3">
              <h3 style={{ fontSize: '16px', fontWeight: 600, color: '#0D0D0D', margin: '0 0 6px' }}>
                {item.title}
                {item.badge && <PulseBadge color="#b0004a" style={{ marginLeft: 8 }}>{item.badge}</PulseBadge>}
              </h3>
              {item.price && <span style={{ fontSize: '20px', fontWeight: 700, color: '#b0004a', flexShrink: 0 }}>{item.price}</span>}
            </div>
    ...
    <div className={className} style={{ textAlign: 'center', padding: '48px 16px' }}>
      {number !== undefined ? (
        <AnimatedCounter
          ...
          prefix={prefix}
          suffix={suffix}
          style={{ fontSize: 56, fontWeight: 700, color: '#b0004a', lineHeight: 1 }}
        />
      ) : (
...
export function DataCard({
  ...
  return (
    <div className={className} style={{ background: '#FFFFFF', borderRadius: 16, border: '1px solid rgba(0,0,0,0.06)', padding: '28px 32px' }}>
      ...
      <div style={{ fontSize: '14px', fontVariantNumeric: 'tabular-nums' }}>
        {rows.map((r) => (
          <div key={r.label} className="flex justify-between" style={{ padding: '8px 0', borderBottom: '1px solid rgba(0,0,0,0.04)', color: r.highlight ? '#b0004a' : 'rgba(0,0,0,0.5)' }}>
            <span>{r.label}</span>
            <span style={{ fontWeight: 600, color: r.highlight ? '#b0004a' : '#0D0D0D' }}>{r.value}</span>
          </div>
        ))}
      </div>
      {footer && (
        <div className="flex justify-between" style={{ marginTop: 12, paddingTop: 12, borderTop: '2px solid rgba(0,0,0,0.12)', fontSize: '16px', fontWeight: 700, color: '#b0004a' }}>
    ...
    >
      <div style={{ padding: '20px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
        ...
        <motion.span
          ...
          style={{ flexShrink: 0, color: '#b0004a', fontSize: '18px', lineHeight: 1 }}
        >
          &#9662;
        </motion.span>
      </div>
...
export function DealPreview({
  ...
  return (
    <div className={className} style={{ background: 'linear-gradient(135deg, #0D0D0D 0%, #2B2A27 100%)', borderRadius: 16, padding: '28px 32px', color: '#fff' }}>
      <span style={{ fontSize: '11px', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#b0004a' }}>{title}</span>
...
export function SideBySideCard({
  ...
  return (
    <div className={className} style={{ display: 'flex', borderRadius: 16, overflow: 'hidden', border: '1px solid rgba(0,0,0,0.06)' }}>
      ...
      <div style={{ flex: 1, padding: '24px', background: '#b0004a' }}>
    ...
    <div ref={ref} className={className} style={{ background: '#FFFFFF', borderRadius: 16, border: '1px solid rgba(0,0,0,0.06)', padding: '28px 32px' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        {messages.slice(0, visibleCount).map((msg, i) => (
          ...
          >
            {msg.role === 'ai' && <span style={{ fontSize: '11px', fontWeight: 600, color: '#b0004a', display: 'block', marginBottom: 4 }}>YULIA</span>}
            {msg.text}
          </motion.div>
        ))}
      ...
      {items.map((item, i) => (
        <ScrollReveal key={i} delay={i * 0.05}>
          <div className="flex gap-4 items-start">
            <span style={{
              fontSize: '14px', fontWeight: 700, color: '#b0004a',
