...
export default function HowItWorksBelow() {
  ...
  const comparisonRows = [
    ...
    { feature: 'Number verification', ai: 'AI generates numbers', smbx: 'AI suggests → you verify → engine calculates' },
  ...
  const leagues = [
    {
      ...
      focus: 'Add-back discovery, SBA qualification, step-by-step guidance through the entire process.',
      verification: 'Add-back verification, owner salary normalization, one-time expense identification. Yulia walks you through each one.',
    ...
    },
    {
      ...
      focus: 'PE buyer mapping, management gap assessment, competitive process preparation.',
      verification: 'Full add-back schedule, working capital normalization, trend analysis, seasonality review — the same Quality of Earnings framework a buyer\'s diligence team will use.',
    ...
    },
    {
      ...
      focus: 'Competitive auction, sensitivity analysis, cross-border structures, board-level documentation.',
      verification: 'Institutional QoE standards — forensic verification, synergy assumption testing, regulatory compliance review. The depth your buyer\'s deal team expects before they write the check.',
  ...
  return (
    <div className={dark ? 'bg-transparent text-[#dadadc]' : 'bg-transparent text-[#1a1c1e]'}>
      <div className="pt-12 pb-24 px-6 md:px-12 max-w-6xl mx-auto">
          ...
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              ...
              <StaggerItem key={stage.num}>
                <div className={`rounded-3xl p-10 h-full ${card}`}>
        ...
        <ScrollReveal>
          <section className="mb-32 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            <div className="space-y-6">
              ...
              <p className={`text-lg leading-relaxed ${muted}`}>Institutional investors pay tens of thousands for real-time terminal access to private market data. For everyone else, the "Information Desert" is real — outdated averages, anecdotal advice, and rules of thumb that vary by 40% between markets.</p>
              <div className={`p-6 rounded-2xl space-y-4 ${card}`}>
                <div className={`flex justify-between items-center pb-4 border-b ${borderColor}`}>
                  <span className={`font-bold ${muted}`}>Traditional Research</span>
                  <span className={`font-bold italic ${dark ? 'text-red-400' : 'text-red-600'}`}>"Good luck"</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className={`font-bold ${emphasis}`}>smbX.ai</span>
                  <span className="text-[#b0004a] font-black">Institutional Grade</span>
                </div>
              </div>
        ...
        <section className="mb-32">
          <ScrollReveal>
            <div className="mb-12">
              ...
              <h2 className="font-headline text-4xl md:text-5xl font-black tracking-tight mb-4">A real deal analysis isn't one thing. <span className="text-[#b0004a]">It's seven.</span></h2>
              <p className={`text-lg max-w-2xl ${muted}`}>Every sentence you type triggers all seven simultaneously. By the time you've described your business, Yulia has already benchmarked it against thousands of comparables.</p>
            </div>
          </ScrollReveal>
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              <div className={`flex-1 p-10 rounded-3xl ${dark ? 'bg-[#2f3133] border border-zinc-800' : 'bg-white border border-[#eeeef0] shadow-sm'}`}>
                <div className="flex justify-between items-start mb-8">
                  <div>
                    <div className={`text-xs font-bold uppercase tracking-widest mb-2 ${muted}`}>Service Sector</div>
                    <h4 className="font-headline text-2xl font-black">Cleaning in Phoenix</h4>
                  </div>
                ...
                <div className="space-y-4 mb-8">
                  ...
                  <p className={`text-sm leading-relaxed ${muted}`}>Population growth driving demand, but labor costs rising faster than contract prices. Margin compression risk must be priced in.</p>
                </div>
              ...
              <div className={`flex-1 p-10 rounded-3xl ${dark ? 'bg-[#2f3133]' : 'bg-[#f3f3f6]'}`}>
                <div className="flex justify-between items-start mb-8">
                  <div>
                    <div className={`text-xs font-bold uppercase tracking-widest mb-2 ${muted}`}>Industrial Sector</div>
                    <h4 className="font-headline text-2xl font-black">Precision Manufacturing</h4>
                  </div>
                ...
                <div className="space-y-4 mb-8">
                  ...
                  <p className={`text-sm leading-relaxed ${muted}`}>Specialized IP and long-term contracts create a moat. Strategic buyers paying a 25% premium over financial sponsors.</p>
                </div>
        ...
        <section className="mb-32">
          <ScrollReveal>
            <div className="mb-12">
              ...
              <h2 className="font-headline text-4xl font-black tracking-tight mb-4">Yulia doesn't give the same advice to every deal.</h2>
              <p className={`text-lg max-w-2xl ${muted}`}>A $400K cleaning company and a $40M manufacturing platform both ask "what's my business worth?" If they get the same answer, the answer is wrong for both of them. Yulia classifies your deal by complexity and adapts everything — language, metrics, analytical depth, buyer strategy, and the rigor of the financial verification.</p>
            </div>
          </ScrollReveal>
              ...
              <StaggerItem key={league.title}>
                <div className={`rounded-2xl overflow-hidden h-full ${card}`}>
                  ...
                  <div className="p-6 space-y-4">
                    <div><p className={`text-[9px] uppercase font-bold mb-1 ${muted}`}>Metric</p><p className="font-bold text-sm">{league.metric}</p></div>
                    <div><p className={`text-[9px] uppercase font-bold mb-1 ${muted}`}>Language</p><p className={`text-sm ${muted}`}>{league.language}</p></div>
                    <div><p className={`text-[9px] uppercase font-bold mb-1 ${muted}`}>Focus</p><p className={`text-sm ${muted}`}>{league.focus}</p></div>
                    <div><p className={`text-[9px] uppercase font-bold mb-1 ${muted}`}>Financial Verification</p><p className={`text-sm ${muted}`}>{league.verification}</p></div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
              <div className={`rounded-2xl p-6 flex items-start gap-4 ${dark ? 'bg-[#b0004a]/10 border border-[#b0004a]/20' : 'bg-[#b0004a]/5 border border-[#b0004a]/15'}`}>
                ...
                <div>
                  ...
                  <p className={`text-sm ${muted}`}>Veterinary, dental, HVAC, managed services, and pest control trade on institutional metrics even at small scale because PE roll-ups are active. Yulia automatically switches to EBITDA for these sectors above $1.5M revenue — regardless of size classification.</p>
                </div>
              </div>
              <div className={`rounded-2xl p-6 flex items-start gap-4 ${dark ? 'bg-[#2f3133] border border-zinc-800' : 'bg-[#f3f3f6] border border-[#eeeef0]'}`}>
                ...
                <div>
                  <h4 className="font-bold mb-1">You never select a tier</h4>
                  <p className={`text-sm ${muted}`}>There's no "Basic vs Pro vs Enterprise" dropdown. Yulia classifies your deal from the financials you share — and you see the classification explained in plain language during the conversation. The deal determines the depth. Not a pricing page.</p>
                </div>
              </div>
            </div>
          </ScrollReveal>
        ...
        {/* ═══ 8. THE MATH ENGINE + VERIFICATION LOOP ═══ */}
        <section className="mb-32">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
            <ScrollReveal className="lg:col-span-5">
              ...
              <div className={`space-y-6 leading-[1.75] ${muted}`}>
                ...
                <p>smbX.ai uses a <span className={`font-semibold ${emphasis}`}>hybrid architecture</span>. Yulia reads your context, interprets your situation, and communicates naturally. But every financial calculation runs through a deterministic engine with standardized formulas that don't guess.</p>
                <p className={`font-bold text-xl border-l-4 border-[#b0004a] pl-6 ${emphasis}`}>The AI decides what to calculate. The engine guarantees the math is right.</p>
              </div>
              {/* Verification Loop */}
              <div className={`mt-10 rounded-2xl p-6 text-white ${dark ? 'bg-[#0f1012]' : 'bg-[#1a1c1e]'}`}>
                ...
                <p className="text-xs text-[#dadadc]/60 mb-5">Every number in your analysis passes through a three-step process. This is why your valuation is defensible — not hallucinated.</p>
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div><p className="text-sm font-semibold">AI Identifies</p><p className="text-xs text-[#dadadc]/50">Yulia scans your financials and flags potential add-backs — personal vehicles, above-market rent, one-time legal fees, family payroll.</p></div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div><p className="text-sm font-semibold">You Verify</p><p className="text-xs text-[#dadadc]/50">AI cannot confirm add-backs. You review each one and confirm or reject. Your judgment, your numbers. Yulia explains why she flagged it — you decide.</p></div>
                  </div>
                  <div className="flex items-start gap-3">
                    ...
                    <div><p className="text-sm font-semibold">Engine Calculates</p><p className="text-xs text-[#dadadc]/50">Only verified add-backs enter the formula. The deterministic engine recalculates SDE/EBITDA, reruns the valuation, and updates every downstream model.</p></div>
                  </div>
                </div>
                <div className="mt-5 pt-4 border-t border-white/10">
                  <p className="text-[10px] text-[#dadadc]/30 italic">This loop repeats for every financial claim. The result: a valuation that survives a buyer's Quality of Earnings review — because it was built the same way.</p>
                </div>
              </div>
            </ScrollReveal>
            <StaggerContainer className="lg:col-span-7 space-y-4">
              <StaggerItem>
                <div className={`rounded-2xl p-6 ${card}`}>
                  ...
                  <div className={`rounded-xl p-4 font-mono text-sm ${codeBg}`}>SDE = Net Income + Owner Salary + D&A + Interest + One-Time Expenses + Verified Add-Backs</div>
                  <p className={`text-xs mt-3 ${muted}`}>Used for owner-operated businesses under $2M. "Verified" means you confirmed each add-back. The AI flagged it; you approved it; the engine counted it.</p>
                </div>
              </StaggerItem>
              <StaggerItem>
                <div className={`rounded-2xl p-6 ${card}`}>
                  ...
                  <div className={`rounded-xl p-4 font-mono text-sm ${codeBg}`}>Adj. EBITDA = Net Income + D&A + Interest + Taxes + Verified Add-Backs − Non-Recurring Income</div>
                  <p className={`text-xs mt-3 ${muted}`}>Used for institutional deals $2M+. GAAP-normalized with working capital adjustments. The number a PE buyer's QoE firm will verify.</p>
                </div>
              </StaggerItem>
              <StaggerItem>
                <div className={`rounded-2xl p-6 ${card}`}>
                  ...
                  <div className="flex gap-4 mt-3">
                    ...
                    <div className="flex items-center gap-2"><span className={`w-3 h-3 rounded-full ${dark ? 'bg-white' : 'bg-[#1a1c1e]'}`}></span><span className={`text-xs ${muted}`}>Conventional: 1.50×</span></div>
                  </div>
                  <p className={`text-xs mt-3 ${muted}`}>Yulia models this at current rates before you write the LOI. If the deal doesn't clear DSCR, she tells you before you waste time.</p>
                </div>
              </StaggerItem>
              <StaggerItem>
                <div className={`rounded-2xl p-6 text-white ${dark ? 'bg-[#0f1012]' : 'bg-[#1a1c1e]'}`}>
                  ...
                  <p className="text-xs text-[#dadadc]/50 mt-3">The base multiple comes from verified comparables. Premiums and discounts are calculated from the seven-dimension analysis — not estimated. Every component is sourced.</p>
                </div>
              </StaggerItem>
            </StaggerContainer>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            <ScrollReveal>
              <div>
                ...
                <div className={`space-y-6 leading-[1.75] ${muted}`}>
                  <p>Most AI tools let you ask anything in any order. That's fine for general questions. It's dangerous for M&A.</p>
                  <p>Drafting an LOI before your financials are normalized means negotiating from a wrong number. Building a CIM before your value readiness is scored means presenting a business with fixable weaknesses exposed. Skipping due diligence coordination means discovering deal-killers at the closing table.</p>
                  <p>smbX.ai enforces a <span className={`font-semibold ${emphasis}`}>gate system</span> — 22 gates across 4 journeys. Each gate has specific completion triggers. Yulia won't advance you until the prerequisite work is done. This isn't a limitation. It's the methodology that prevents the mistakes most first-time deal participants make.</p>
                </div>
              </div>
            </ScrollReveal>
            <StaggerContainer className="space-y-4">
              ...
              <StaggerItem>
                <div className={`rounded-2xl p-6 ${card}`}>
                  <div className="flex items-center gap-3 mb-4"><span className="material-symbols-outlined text-[#b0004a]">storefront</span><h4 className="font-bold">Sell Journey — 6 Gates</h4></div>
                  <div className="flex gap-2">
                    {[
                      { id: 'S0', label: 'Profile', free: true },
                      { id: 'S1', label: 'Financials', free: true },
                      { id: 'S2', label: 'Valuation', free: false },
                      { id: 'S3', label: 'Prepare', free: false },
                      { id: 'S4', label: 'Negotiate', free: false },
                      { id: 'S5', label: 'Close', free: false },
                    ].map((g) => (
                      <div key={g.id} className="flex-1 text-center">
                        <div className={`rounded-lg py-2 px-1 mb-1 ${g.free ? 'bg-[#006630]/10' : 'bg-[#b0004a]/10'}`}>
                          <span className={`text-[10px] font-bold ${g.free ? 'text-[#006630]' : 'text-[#b0004a]'}`}>{g.id}</span>
                        </div>
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    <div className="w-3 h-3 rounded-sm bg-[#006630]/10"></div><span className={`text-[9px] ${muted}`}>Free</span>
                    <div className="w-3 h-3 rounded-sm bg-[#b0004a]/10 ml-2"></div><span className={`text-[9px] ${muted}`}>Execution fee</span>
                  </div>
                </div>
              </StaggerItem>
              {/* Buy gates */}
              <StaggerItem>
                <div className={`rounded-2xl p-6 ${card}`}>
                  <div className="flex items-center gap-3 mb-4"><span className="material-symbols-outlined text-[#b0004a]">shopping_bag</span><h4 className="font-bold">Buy Journey — 6 Gates</h4></div>
                  <div className="flex gap-2">
                    {[
                      { id: 'B0', label: 'Thesis', free: true },
                      { id: 'B1', label: 'Source', free: true },
                      { id: 'B2', label: 'Underwrite', free: false },
                      { id: 'B3', label: 'Diligence', free: false },
                      { id: 'B4', label: 'Negotiate', free: false },
                      { id: 'B5', label: 'Close', free: false },
                    ].map((g) => (
                      <div key={g.id} className="flex-1 text-center">
                        <div className={`rounded-lg py-2 px-1 mb-1 ${g.free ? 'bg-[#006630]/10' : 'bg-[#b0004a]/10'}`}>
                          <span className={`text-[10px] font-bold ${g.free ? 'text-[#006630]' : 'text-[#b0004a]'}`}>{g.id}</span>
              ...
              <StaggerItem>
                <div className="grid grid-cols-2 gap-4">
                  <div className={`rounded-2xl p-6 ${card}`}>
                    <div className="flex items-center gap-3 mb-4"><span className="material-symbols-outlined text-[#b0004a]">trending_up</span><h4 className="font-bold text-sm">Raise — 6 Gates</h4></div>
                    <div className="flex gap-1">
                      {['R0', 'R1', 'R2', 'R3', 'R4', 'R5'].map((id) => (
                        <div key={id} className="flex-1 text-center">
                          <div className={`rounded-lg py-1.5 mb-1 ${['R0', 'R1'].includes(id) ? 'bg-[#006630]/10' : 'bg-[#b0004a]/10'}`}>
                            <span className={`text-[9px] font-bold ${['R0', 'R1'].includes(id) ? 'text-[#006630]' : 'text-[#b0004a]'}`}>{id}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className={`rounded-2xl p-6 ${card}`}>
                    <div className="flex items-center gap-3 mb-4"><span className="material-symbols-outlined text-[#b0004a]">merge</span><h4 className="font-bold text-sm">Integrate — 4 Gates</h4></div>
                    <div className="flex gap-1">
                      {['I0', 'I1', 'I2', 'I3'].map((id) => (
                        <div key={id} className="flex-1 text-center">
                          <div className={`rounded-lg py-1.5 mb-1 ${id === 'I0' ? 'bg-[#006630]/10' : 'bg-[#b0004a]/10'}`}>
                            <span className={`text-[9px] font-bold ${id === 'I0' ? 'text-[#006630]' : 'text-[#b0004a]'}`}>{id}</span>
                          </div>
                        </div>
                      ))}
                    </div>
              ...
              {/* Gate enforcement callout */}
              <StaggerItem>
                <div className={`rounded-2xl p-6 ${subtleBg}`}>
                  <h4 className="font-bold text-sm mb-3 flex items-center gap-2"><span className="material-symbols-outlined text-[#b0004a] text-lg">lock</span>What "gate enforcement" means in practice</h4>
                  <div className={`space-y-3 text-sm ${muted}`}>
                    <p>→ Yulia won't generate a CIM until your Value Readiness Report is scored and your financials are normalized.</p>
                    <p>→ She won't draft an LOI until the acquisition model clears DSCR at current lending rates.</p>
                    <p>→ She won't advance to closing coordination until due diligence items are tracked and resolved.</p>
                    <p>→ Each gate has specific completion triggers — not a checklist you click through, but conditions Yulia verifies from the conversation and the data.</p>
                  </div>
                </div>
              </StaggerItem>
            </StaggerContainer>
