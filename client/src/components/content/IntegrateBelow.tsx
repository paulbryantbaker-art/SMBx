    ...
    <div className={dark ? 'bg-transparent text-[#f9f9fc]' : 'bg-transparent text-[#1a1c1e]'}>
      <div className="pt-12 pb-24 px-6 md:px-12 max-w-6xl mx-auto">
          ...
          <div className="lg:col-span-7">
            <ScrollReveal>
              <div className="flex items-center gap-2 mb-8">
                <span className="inline-block px-3 py-1 bg-[#A85860]/10 text-[#A85860] text-[10px] font-black uppercase tracking-[0.2em] rounded-sm">Integrate</span>
            ...
            <ScrollReveal y={40} delay={0.1}>
              <h1 className="font-headline font-black text-5xl md:text-6xl tracking-tighter leading-[0.92] mb-8">
                The first 180 days determine <span className="text-[#A85860]">everything.</span>
              </h1>
            </ScrollReveal>
            <ScrollReveal delay={0.2}>
              <div className={`space-y-6 text-xl editorial max-w-xl ${dark ? 'text-[#dadadc]/80' : 'text-[#5d5e61]'}`}>
                ...
                <p className={`font-bold border-l-4 border-[#A85860] pl-6 text-2xl italic ${dark ? 'text-[#f9f9fc]' : 'text-[#1a1c1e]'}`}>Yulia builds your value creation plan from what due diligence actually revealed. Not a template. Your plan.</p>
              </div>
            </ScrollReveal>
          </div>
        ...
        <section className="mb-24">
          <ScrollReveal>
            <div className="mb-12">
              <span className="text-[#A85860] font-bold uppercase tracking-widest text-xs block mb-3">The Framework</span>
              ...
              <StaggerItem key={p.phase}>
                <div className={`rounded-2xl p-8 hover:shadow-lg transition-all ${dark ? 'bg-[#2f3133] border border-zinc-800' : 'bg-white border border-[#eeeef0]'}`}>
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-[#A85860] font-bold text-xs uppercase tracking-widest">Phase {p.phase}</span>
                    <span className="text-[10px] bg-[#A85860]/10 text-[#A85860] px-2 py-1 rounded font-bold">{p.time}</span>
                  </div>
                  ...
                  <div className={`h-1.5 rounded-full overflow-hidden ${dark ? 'bg-zinc-700' : 'bg-[#f3f3f6]'}`}>
                    <div className="h-full bg-[#A85860] rounded-full" style={{ width: `${p.progress}%` }} />
                  </div>
                </div>
              </StaggerItem>
            ))}
        ...
        <section className="mb-24">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
            <ScrollReveal className="lg:col-span-5">
              <span className="text-[#A85860] font-bold uppercase tracking-widest text-xs block mb-3">Live Monitoring</span>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  <div key={metric.title} className={`rounded-2xl p-6 ${dark ? 'bg-[#2f3133] border border-zinc-800' : 'bg-white border border-[#eeeef0]'}`}>
                    <span className="material-symbols-outlined text-[#A85860] text-xl mb-3">{metric.icon}</span>
                ...
                <div className={`sm:col-span-2 rounded-2xl p-6 ${dark ? 'bg-[#2f3133] border border-zinc-800' : 'bg-white border border-[#eeeef0]'}`}>
                  <span className="material-symbols-outlined text-[#A85860] text-xl mb-3">handshake</span>
        ...
        <ScrollReveal>
          <section className="mb-24">
            <div className={`rounded-3xl p-10 md:p-16 text-white ${dark ? 'bg-[#0f1012]' : 'bg-[#1a1c1e]'}`}>
              <div className="mb-12">
                <span className="text-[#A85860] font-bold uppercase tracking-widest text-xs block mb-3">Avoid These</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                {[
                  ...
                  <div key={mistake.num} className="border-t border-white/15 pt-6">
                    <span className="text-[#A85860] font-black text-2xl mb-3 block">{mistake.num}</span>
                    <h4 className="font-bold mb-2">{mistake.title}</h4>
                    <p className="text-sm text-[#dadadc]/70">{mistake.desc}</p>
                  </div>
                ))}
              </div>
              <div className="bg-[#A85860] rounded-2xl p-8">
