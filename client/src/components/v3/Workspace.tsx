...
function ChatPane({ mode, setMode, thread, draft, setDraft, send, inputRef, onSlash }: ChatPaneProps) {
  return (
    <div style={vw.chat}>
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
        <span className="mono" style={{
          fontSize: 10.5, color: accent ? "var(--go)" : "var(--ink-4)",
          letterSpacing: "0.1em",
        }}>
          {tag}
        </span>
        <svg width="12" height="12" viewBox="0 0 14 14" fill="none" style={{
          color: accent ? "var(--go)" : "var(--ink-3)",
  ...
  return (
    <div className="fade-up" style={{ display: "flex", gap: 10, marginBottom: 16 }}>
      <div style={{
        ...
        borderRadius: 5,
        background: isY ? "var(--go-soft)" : "var(--surface-2)",
        border: isY ? "1px solid var(--go-ring)" : "1px solid var(--line-2)",
        color: isY ? "var(--go)" : "var(--ink-3)",
    ...
    <form style={vw.composer} onSubmit={(e) => { e.preventDefault(); send(); }}>
      <div style={vw.composerBody}>
        ...
        <div style={vw.composerFoot}>
          ...
          <button type="submit" disabled={disabled} className="btn btn-cta" style={{ padding: "6px 12px", fontSize: 12, opacity: disabled ? 0.4 : 1 }}>
            Send
...
const vw: Record<string, CSSProperties> = {
  ...
  doorAccent: {
    background: "linear-gradient(135deg, var(--go-soft), transparent 60%), var(--surface)",
