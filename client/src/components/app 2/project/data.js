/* ═══════════════════════════════════════════════════════════
   Demo data — portfolios, deals, artifacts, chat scripts
   ═══════════════════════════════════════════════════════════ */

/* Portfolios — top level. Most users have 1; advisors/IBs have many. */
window.PORTFOLIOS = [
...
window.DEALS = [
  {
    ...
    stage: 'dd',              // sourcing | rundown | dd | loi | closing
    stageIdx: 2,
    fit: 94,
    score: 83,
    status: 'pursue',         // pursue | hold | pass
    tone: 'ok',
...
/* Canned chat responses per deal. Yulia responds based on keywords. */
window.CHAT_SEEDS = {
  atlas: [
...
/* Yulia reply generator — keyword matched, falls back to generic.
   Can return { text, cards, spawnTabs } where spawnTabs = [{kind, dealId, label}, ...]
   to create multiple new deal tabs at once. */
window.yuliaReply = function(text, deal, portfolio) {
  const t = text.toLowerCase();
  const port = portfolio || window.PORTFOLIOS[0];
  const portDeals = window.DEALS.filter(d => port.dealIds.includes(d.id));

  /* "Model DCF on Atlas, Summit, Benchmark and compare" — detect multi-deal + model intent */
  ...
  if (wantsModel && (mentioned.length >= 2 || (wantsCompare && mentioned.length === 0))) {
    const targets = mentioned.length >= 2 ? mentioned : portDeals.filter(d => d.score).slice(0, 3);
    const spawnTabs = targets.map(d => ({ kind: 'model', dealId: d.id, label: `${d.name} · DCF` }));
    if (wantsCompare) spawnTabs.push({ kind: 'compare', dealIds: targets.map(d => d.id), label: `Compare · ${targets.map(d => d.name.split(' ')[0]).join(' + ')}` });
    return {
      ...
      cards: null,
      spawnTabs,
      progress: spawnTabs.map(t => t.label),
    };
  }
...
/* Data rooms — scoped to a deal. Each item carries status:
     review  — seller uploaded, needs your eyes
     action  — you owe a response / sign-off / question
     sealed  — reviewed, closed out, archived for audit */
window.DATA_ROOMS = [
...
/* Docs the user has created — analyses, memos, drafts, infographics. */
window.MY_DOCS = [
...
/* Docs shared to the user (not via data room — think co-investors, advisors). */
window.SHARED_DOCS = [
...
/* Templates Yulia can generate on request. */
window.TEMPLATES = [
