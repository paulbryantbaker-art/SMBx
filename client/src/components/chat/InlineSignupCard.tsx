...
export default function InlineSignupCard({ sessionId, onDismiss, canDismiss = true }: InlineSignupCardProps) {
  ...
  if (success) {
    return (
      <div className="self-center w-full max-w-[95%] bg-white rounded-2xl px-6 py-8 text-center" style={{ boxShadow: '0 2px 8px rgba(0,0,0,.07), 0 1px 2px rgba(0,0,0,.04)', borderLeft: '3px solid #D4714E' }}>
        ...
        <p className="text-lg font-semibold text-[#1a1918] m-0">You're all set</p>
        <p className="text-sm text-[#5e5d59] m-0 mt-1">Taking you to your workspace...</p>
      </div>
    );
  }

  return (
    <div className="self-center w-full max-w-[95%] bg-white rounded-2xl px-6 py-6" style={{ boxShadow: '0 2px 8px rgba(0,0,0,.07), 0 1px 2px rgba(0,0,0,.04)', borderLeft: '3px solid #D4714E' }}>
      {canDismiss && onDismiss && (
        <button
          onClick={onDismiss}
          className="absolute top-3 right-3 text-[#A9A49C] hover:text-[#1a1918] bg-transparent border-none cursor-pointer p-1"
          type="button"
        >
      ...
      <h3 className="text-lg font-semibold text-[#1a1918] m-0 mb-1" style={{ fontFamily: "'Figtree', system-ui, sans-serif" }}>
        Save your progress and continue with Yulia
      </h3>
      <p className="text-[15px] text-[#3d3d3a] m-0 mb-5 leading-relaxed">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        {mode === 'signup' && (
          <input
            ...
            required
            className="w-full px-4 py-3 rounded-xl border border-[rgba(0,0,0,0.08)] text-base text-[#1a1918] outline-none transition-colors focus:border-[#D4714E]"
            style={{ fontFamily: "'Figtree', system-ui, sans-serif", boxShadow: 'inset 0 0 0 0 transparent' }}
          />
        )}
        <input
          type="email"
        ...
        {error && (
          <p className="text-sm text-[#7a3048] m-0">{error}</p>
        )}

        <button
          type="submit"
          disabled={loading}
          className="w-full py-3 rounded-xl border-none bg-[#D4714E] text-white text-base font-semibold cursor-pointer hover:bg-[#B85A3A] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
          style={{ fontFamily: "'Figtree', system-ui, sans-serif", height: '48px' }}
        >
      ...
      <button
        ...
        disabled={loading}
        className="w-full py-3 rounded-xl border border-[rgba(0,0,0,0.08)] bg-white text-[#1a1918] text-base font-medium cursor-pointer hover:bg-[#F5F5F5] disabled:opacity-50 transition-colors flex items-center justify-center gap-2"
        style={{ fontFamily: "'Figtree', system-ui, sans-serif", height: '48px' }}
        type="button"
      >
      ...
      {/* Toggle mode */}
      <p className="text-[14px] text-[#5e5d59] text-center m-0 mt-4">
        {mode === 'signup' ? (
