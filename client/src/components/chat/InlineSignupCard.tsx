...
export default function InlineSignupCard({ sessionId, onDismiss, canDismiss = true }: InlineSignupCardProps) {
  ...
  if (success) {
    return (
      <div className="self-center w-full max-w-[95%] bg-white rounded-2xl px-6 py-8 text-center" style={{ boxShadow: '0 2px 8px rgba(0,0,0,.07), 0 1px 2px rgba(0,0,0,.04)', borderLeft: '3px solid #A03050' }}>
        <div className="w-10 h-10 rounded-full bg-[#A03050] text-white flex items-center justify-center mx-auto mb-3">
  ...
  return (
    <div className="self-center w-full max-w-[95%] bg-white rounded-2xl px-6 py-6" style={{ boxShadow: '0 2px 8px rgba(0,0,0,.07), 0 1px 2px rgba(0,0,0,.04)', borderLeft: '3px solid #A03050' }}>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3">
        {mode === 'signup' && (
          <input
            ...
            required
            className="w-full px-4 py-3 rounded-xl border border-[rgba(0,0,0,0.08)] text-base text-[#0D0D0D] outline-none transition-colors focus:border-[#A03050]"
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full py-3 rounded-xl border-none bg-[#A03050] text-white text-base font-semibold cursor-pointer hover:bg-[#802040] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
      ...
      <p className="text-[14px] text-[#6E6A63] text-center m-0 mt-4">
        {mode === 'signup' ? (
          <>Already have an account? <button onClick={() => { setMode('login'); setError(null); }} className="text-[#A03050] font-semibold bg-transparent border-none cursor-pointer underline" style={{ fontFamily: 'inherit', fontSize: 'inherit' }} type="button">Sign in</button></>
        ) : (
          <>New here? <button onClick={() => { setMode('signup'); setError(null); }} className="text-[#A03050] font-semibold bg-transparent border-none cursor-pointer underline" style={{ fontFamily: 'inherit', fontSize: 'inherit' }} type="button">Create account</button></>
        )}
      </p>
    </div>
  );
