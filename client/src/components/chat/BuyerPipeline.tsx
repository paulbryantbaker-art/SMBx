...
/* Cowork earth-tone status chips — sage/sky/peach instead of green/blue/yellow. */
const STATUS_COLORS: Record<string, { bg: string; text: string; label: string }> = {
  pursuing:  { bg: 'bg-cactus/40 border-cactus',  text: 'text-[#1f4a3a]', label: 'Pursuing' },
  reviewing: { bg: 'bg-sky/15 border-sky/40',     text: 'text-[#2f5a85]', label: 'Reviewing' },
  flagged:   { bg: 'bg-peach/50 border-peach',    text: 'text-[#8a3d23]', label: 'New' },
};

const READINESS_TIERS: Record<string, { color: string; label: string }> = {
  hot:  { color: 'text-[#B85A3A]', label: 'Hot' },     /* clay */
  warm: { color: 'text-[#D4714E]', label: 'Warm' },    /* clay lighter */
  cold: { color: 'text-[#87867f]', label: 'Cold' },    /* gray-500 */
};
  ...
  if (!data?.hasThesis) {
    return (
      <div className="p-8 text-center">
        ...
        <h2 className="text-lg font-bold text-[#1a1918] mb-2">No Acquisition Thesis Yet</h2>
        <p className="text-sm text-[#5e5d59]">Tell Yulia what you're looking to buy and she'll build your thesis and start matching opportunities.</p>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-2xl p-6 mb-6" style={{ border: '1px solid #e8e6dc' }}>
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-xl font-bold text-[#1a1918] m-0 mb-1">Your Acquisition Thesis</h1>
          ...
          <span className="text-sm font-bold text-[#D4714E]">{targets.length} matches</span>
        </div>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-3 gap-4 mb-6">
          <div className="bg-cactus/40 rounded-xl p-3 text-center border border-cactus/60">
            <p className="text-xl font-bold text-[#1f4a3a]">{stats.pursuing}</p>
            <p className="text-xs text-[#3f6b5d]">Pursuing</p>
          </div>
          <div className="bg-sky/15 rounded-xl p-3 text-center border border-sky/25">
            <p className="text-xl font-bold text-[#2f5a85]">{stats.reviewing}</p>
            <p className="text-xs text-[#2f5a85]">Reviewing</p>
          </div>
          <div className="bg-peach/50 rounded-xl p-3 text-center border border-peach/70">
            <p className="text-xl font-bold text-[#8a3d23]">{stats.flagged}</p>
            <p className="text-xs text-[#8a3d23]">New</p>
          </div>
        </div>
      )}

      {/* Targets */}
      <h2 className="text-lg font-bold text-[#1a1918] mb-3">Opportunities</h2>
      {targets.length === 0 ? (
        <div className="bg-white rounded-xl p-6 text-center" style={{ border: '1px solid #e8e6dc' }}>
          <p className="text-sm text-[#5e5d59]">No matches yet. Yulia will scan for opportunities matching your thesis.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {targets.map(target => {
            ...
            return (
              <div key={target.id} className="bg-white rounded-xl p-5" style={{ border: '1px solid #e8e6dc' }}>
                <div className="flex items-start justify-between gap-4">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="text-sm font-bold text-[#1a1918] m-0 truncate">{target.company_name}</h3>
                    ...
                    <div className="flex items-center gap-4 mt-2 text-xs text-[#3d3d3a]">
                    ...
                    {target.conviction_check && (
                      <div className="mt-2 pt-2 border-t border-[#F5F5F5]">
                        <p className="text-[10px] font-semibold uppercase tracking-wider text-[#5e5d59] mb-1">Quick Conviction</p>
                        <div className="space-y-0.5">
                          {target.conviction_check.checks.map((check, i) => (
                            <div key={i} className="flex items-center gap-1.5 text-[11px]">
                              <span className={check.pass === true ? 'text-[#3f6b5d]' : check.pass === false ? 'text-[#c46686]' : 'text-[#D4714E]'}>
                        ...
                        <p className={`text-[10px] font-semibold mt-1 ${
                          target.conviction_check.verdict === 'pursue' ? 'text-[#3f6b5d]'
                          : target.conviction_check.verdict === 'investigate' ? 'text-[#8a3d23]'
                          : 'text-[#c46686]'
                        }`}>
                    ...
                    <div className="flex items-center gap-1.5 mt-2 text-[10px] text-[#A9A49C]">
                      ...
                      {target.source_url && (
                        <a href={target.source_url} target="_blank" rel="noopener" className="text-[#D4714E] hover:underline">View listing</a>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-1.5 shrink-0">
                    {target.buyer_status !== 'pursuing' && (
                      <button
                        ...
                        className="text-[11px] font-semibold px-3 py-1.5 rounded-lg bg-[#D4714E] text-white border-0 cursor-pointer hover:bg-[#B85A3A] transition-colors disabled:opacity-50"
                        type="button"
                      >
                        Pursue
                      </button>
                    ...
                    {target.buyer_status !== 'reviewing' && target.buyer_status !== 'pursuing' && (
                      <button
                        ...
                        disabled={updatingId === target.id}
                        className="text-[11px] font-semibold px-3 py-1.5 rounded-lg bg-[#F5F5F5] text-[#3d3d3a] border-0 cursor-pointer hover:bg-[#e8e6dc] transition-colors disabled:opacity-50"
                        type="button"
                      >
                        Review
                      </button>
                    ...
                    <button
                      ...
                      disabled={updatingId === target.id}
                      className="text-[11px] font-semibold px-3 py-1.5 rounded-lg bg-transparent text-[#A9A49C] border border-[#e8e6dc] cursor-pointer hover:bg-fig/15 hover:text-[#7a3048] hover:border-fig/40 transition-colors disabled:opacity-50"
                      type="button"
                    >
                      Pass
                    </button>
