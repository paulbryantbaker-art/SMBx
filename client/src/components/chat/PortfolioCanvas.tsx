  ...
  return (
    <div className={pad}>
      <div className={maxW}>
        ...
        {isActive && (
          <div className="mb-4 bg-[#FAFAFA] rounded-xl p-3 border border-[#C4687A]/10">
            ...
            {portfolio.stage_progress?.pct != null && (
              <div className="w-full h-1.5 bg-[#EBE7DF] rounded-full overflow-hidden">
                <div
                  className="h-full bg-[#C4687A] rounded-full transition-all duration-700"
    ...
    <button
      ...
      className={`px-3 py-1 rounded-full text-[11px] font-medium border transition-colors cursor-pointer shrink-0 ${
        active
          ...
          : 'bg-white text-[#6E6A63] border-[rgba(0,0,0,0.08)] hover:border-[#C4687A]'
      }`}
    >
      {label}
    </button>
...
function CandidateCard({ candidate: c, onSelect, onStatusChange }: {
  ...
  return (
    <div
      onClick={onSelect}
      className="bg-white rounded-xl p-3 border border-[rgba(0,0,0,0.06)] hover:border-[#C4687A]/30 cursor-pointer transition-all"
    >
        ...
        <div className="px-5 py-4 space-y-5">
            ...
            <div className="space-y-1.5 text-xs">
              {c.website_url && (
                <DetailRow label="Website">
                  <a href={c.website_url} target="_blank" rel="noopener noreferrer" className="text-[#C4687A] hover:underline truncate">
          ...
          {c.enrichment_tier < 3 && (
            <button
              ...
              disabled={enriching}
              className="w-full py-2.5 rounded-xl text-xs font-semibold bg-[#C4687A] text-white border-0 cursor-pointer hover:bg-[#A85568] transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {enriching ? (
                <>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-3 py-2 rounded-lg border border-[rgba(0,0,0,0.08)] text-xs bg-white text-[#0D0D0D] outline-none focus:border-[#C4687A] resize-none"
            />
            {noteDraft !== (c.user_notes || '') && (
              <button
                onClick={onNoteSave}
                className="mt-1.5 px-3 py-1 rounded-lg text-[10px] font-medium bg-[#C4687A] text-white border-0 cursor-pointer hover:bg-[#A85568] transition-colors"
              >
                Save Notes
              </button>
            )}
