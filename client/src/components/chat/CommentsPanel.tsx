    ...
    <div className="flex flex-col h-full bg-white" style={{ borderLeft: '1px solid rgba(0,0,0,0.08)', width: 320 }}>
      ...
      <div className="shrink-0 flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
        <div className="flex items-center gap-2">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#B5636B" strokeWidth="2">
            <path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z" />
          </svg>
          <span className="text-sm font-semibold text-[#0D0D0D]">Comments</span>
          {active.length > 0 && (
            <span className="text-[10px] font-bold bg-[#B5636B] text-white rounded-full w-5 h-5 flex items-center justify-center">
              {active.length}
            </span>
          )}
        </div>
      ...
      {resolved.length > 0 && (
        <div className="shrink-0 px-4 py-2" style={{ borderBottom: '1px solid #EBE7DF' }}>
          <button
            ...
            className="text-[11px] font-medium text-[#6E6A63] bg-transparent border-0 cursor-pointer hover:text-[#B5636B] transition-colors p-0"
          >
      ...
      <div className="flex-1 overflow-y-auto min-h-0 px-4 py-3 space-y-3">
        {loading && (
          <div className="flex items-center justify-center py-8">
            <div className="w-5 h-5 border-2 border-[#B5636B] border-t-transparent rounded-full animate-spin" />
          </div>
        )}
          ...
          >
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex items-center gap-1.5">
                <div className="w-5 h-5 rounded-full bg-[#B5636B] text-white flex items-center justify-center text-[9px] font-bold shrink-0">
            ...
            {comment.section_ref && (
              <div className="text-[10px] font-medium text-[#B5636B] mb-1">
            ...
            {!comment.resolved && (
              <button
                ...
                className="mt-2 text-[10px] font-medium text-[#6E6A63] bg-transparent border-0 cursor-pointer hover:text-[#B5636B] p-0"
              >
                Resolve
              </button>
            )}
        ...
        <div className="rounded-lg overflow-hidden" style={{ border: '1px solid rgba(0,0,0,0.08)' }}>
          ...
          <div className="flex items-center justify-between px-3 py-1.5 bg-[#FAFAF8]" style={{ borderTop: '1px solid #EBE7DF' }}>
            ...
            <button
              ...
              className="text-[11px] font-semibold text-white bg-[#B5636B] px-3 py-1 rounded border-0 cursor-pointer hover:bg-[#9A5058] transition-colors disabled:opacity-40 disabled:cursor-default"
            >
              {submitting ? '...' : 'Send'}
            </button>
          </div>
