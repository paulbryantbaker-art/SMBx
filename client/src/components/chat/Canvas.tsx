  ...
  const handleExport = async (format: 'pdf' | 'docx' | 'xlsx') => {
    ...
    w.document.write(`
      ...
      <style>
        ...
        h1 { font-size: 24px; border-bottom: 2px solid #B5636B; padding-bottom: 8px; }
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="shrink-0 flex items-center justify-between px-4 py-2.5" style={{ borderBottom: '1px solid rgba(0,0,0,0.08)' }}>
        <div className="flex items-center gap-2 min-w-0">
          <div className="w-6 h-6 rounded bg-[#B5636B] text-white flex items-center justify-center shrink-0">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold border-0 cursor-pointer transition-colors ${
                filed
                  ...
                  : 'bg-[#B5636B] text-white hover:bg-[#9A5058]'
              } disabled:opacity-60`}
            >
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          {showContent && !isMarkdownMode && data?.content?.markdown && deliverableId && (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold border-0 cursor-pointer transition-colors ${
                editMode ? 'bg-[#B5636B] text-white' : 'bg-[#F5F5F5] text-[#3D3B37] hover:bg-[#EBE7DF]'
              }`}
            >
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          {showContent && deliverableId && !isMarkdownMode && (
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold border-0 cursor-pointer transition-colors ${
                commentsOpen ? 'bg-[#B5636B] text-white' : 'bg-[#F5F5F5] text-[#3D3B37] hover:bg-[#EBE7DF]'
              }`}
            >
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        {!isMarkdownMode && !loading && data?.status === 'generating' && (
          <div className="flex items-center justify-center p-12">
            <div className="flex flex-col items-center gap-3 text-center">
              <div className="w-12 h-12 border-2 border-[#B5636B] border-t-transparent rounded-full animate-spin" />
        ...
        {!isMarkdownMode && !loading && (error || !data) && (
          <div className="flex items-center justify-center p-12">
            <div className="flex flex-col items-center gap-3 text-center">
              ...
              <button onClick={fetchData} className="text-sm font-semibold text-[#B5636B] bg-transparent border-0 cursor-pointer hover:underline">
                Try again
              </button>
            </div>
          </div>
  ...
  if (content.sections && Array.isArray(content.sections)) {
    return (
      <div className="space-y-6">
        <h1 className="text-xl font-bold text-[#0D0D0D] m-0 pb-2" style={{ borderBottom: '2px solid #B5636B' }}>
          {name}
        </h1>
        {content.summary && (
          <div className="bg-[#FFF8F5] rounded-xl px-5 py-4" style={{ border: '1px solid rgba(186,60,96,.15)' }}>
            <p className="text-sm font-semibold text-[#B5636B] m-0 mb-1">Summary</p>
          ...
          <nav className="space-y-1">
            {sections.map((s, i) => (
              <button
                ...
                className={`block w-full text-left text-[12px] leading-tight bg-transparent border-0 cursor-pointer hover:text-[#B5636B] transition-colors p-0 ${
