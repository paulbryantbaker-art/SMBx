  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <input
        ...
        style={{
          background: `linear-gradient(to right, #C4687A 0%, #C4687A ${((value - min) / (max - min)) * 100}%, rgba(0,0,0,0.08) ${((value - min) / (max - min)) * 100}%, rgba(0,0,0,0.08) 100%)`,
        }}
      />
...
export default function InteractiveModel({ content }: InteractiveModelProps) {
  ...
  const SCENARIOS: { key: ScenarioKey; label: string; color: string }[] = [
    ...
    { key: 'custom', label: 'Custom', color: '#C4687A' },
  ];
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {savedScenarios.map((s, i) => (
          <div key={i} className="flex items-center gap-1 bg-[#F5F5F5] rounded-full pl-3 pr-1 py-1">
            <button
              ...
              className="text-[10px] font-semibold text-[#3D3B37] bg-transparent border-0 cursor-pointer hover:text-[#C4687A] p-0"
            >
              {s.name}
            </button>
            <button
        ...
        {showSaveInput ? (
          <div className="flex items-center gap-1">
            ...
            />
            <button onClick={saveCurrentScenario} className="text-[10px] font-semibold text-[#C4687A] bg-transparent border-0 cursor-pointer">Save</button>
          ...
          <button
            ...
            className="text-[10px] font-medium text-[#6E6A63] bg-transparent border-0 cursor-pointer hover:text-[#C4687A] flex items-center gap-1"
          >
