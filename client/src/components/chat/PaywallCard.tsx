    ...
    <div className="self-start max-w-[90%] min-w-0">
      <div className="bg-white rounded-[20px] px-6 py-5 overflow-hidden" style={{ boxShadow: '0 2px 12px rgba(186,60,96,.12)', border: '1px solid rgba(186,60,96,.2)' }}>
        ...
        <div className="flex items-start gap-3 mb-4">
          <div className="w-10 h-10 rounded-full bg-[#BA3C60] text-white flex items-center justify-center shrink-0" style={{ boxShadow: '0 2px 6px rgba(186,60,96,.2)' }}>
        ...
        <div className="space-y-2 mb-4">
          {paywall.valueProps.map((prop, i) => (
            <div key={i} className="flex items-start gap-2">
              <svg className="w-4 h-4 text-[#BA3C60] mt-0.5 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12" /></svg>
        ...
        {paywall.whatYouGet && paywall.whatYouGet.length > 0 && (
          <div className="mb-4">
            <button
              ...
              className="text-[13px] text-[#BA3C60] font-semibold bg-transparent border-0 cursor-pointer p-0 hover:opacity-80"
            >
            ...
            {showDetails && (
              <div className="mt-2 pl-1 space-y-1.5">
                {paywall.whatYouGet.map((item, i) => (
                  <div key={i} className="flex items-start gap-2">
                    <span className="text-[#BA3C60] text-xs mt-1 shrink-0">●</span>
