    ...
    >
      ...
      <div className="px-4 mb-6 mt-2">
        <button
          ...
          type="button"
        >
          <svg className="w-4 h-4 text-[#C25572]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <button
                  ...
                  type="button"
                >
                  <span className={isActive ? 'text-[#C25572]' : 'text-[rgba(0,0,0,0.25)]'}>{item.icon}</span>
                  {item.label}
                </button>
              );
            })}
                  ...
                  return (
                    <div key={group.dealId} className="mb-1">
                      ...
                      <div className="flex items-center gap-2 px-3 py-2">
                        ...
                        {group.currentGate && (
                          <span className="text-[9px] font-bold text-[#C25572] bg-[#FFF0EB] px-[5px] py-[1px] rounded-[3px] shrink-0">
                            {group.currentGate}
                          </span>
                        )}
                        <button
                          onClick={(e) => { e.stopPropagation(); handleNewDealChat(group.dealId); }}
                          className="w-[20px] h-[20px] rounded-full border-none cursor-pointer flex items-center justify-center text-[rgba(0,0,0,0.25)] hover:text-[#C25572] hover:bg-[#FFF0EB] transition-colors shrink-0 bg-transparent p-0"
                          type="button"
