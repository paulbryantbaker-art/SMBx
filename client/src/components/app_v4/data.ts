/**
 * Mock data for the v4 rebuild.
 *
 * Ported verbatim from `claude_design/app/project/data.js` with proper
 * TypeScript types. No `window.*` globals — everything is exported.
 *
 * When we wire up real data (later session), this module is the boundary
 * we'll swap — components import from here, not from APIs directly.
 */
...
export type Stage = 'sourcing' | 'rundown' | 'dd' | 'loi' | 'closing';
export type DealStatus = 'pursue' | 'hold' | 'pass';
export type Tone = 'ok' | 'warn' | 'flag';
export type DimTone = 'green' | 'amber' | 'red';
export type DocKind = 'spreadsheet' | 'doc' | 'pdf' | 'zip';
export type RoomStatus = 'review' | 'action' | 'sealed';
export type MyDocKind = 'memo' | 'model' | 'analysis' | 'infographic';
export type TemplateKind = 'loi' | 'memo' | 'model' | 'legal' | 'deck' | 'doc';
...
export type PinnedKind = 'rundown' | 'dd' | 'loi' | 'model' | 'compare';
...
export const DEALS: Deal[] = [
  {
    id: 'atlas',
    name: 'Atlas Air',
    sub: 'Commercial HVAC · Fort Worth, TX',
    stage: 'dd',
    stageIdx: 2,
    fit: 94,
    score: 83,
  ...
  {
    id: 'benchmark',
    name: 'Benchmark Mechanical',
    sub: 'Service HVAC · Dallas, TX',
    stage: 'loi',
    stageIdx: 3,
    fit: 88,
    score: 81,
  ...
  },
  {
    id: 'ridge',
    name: 'Ridge Plumbing',
    sub: 'Residential plumbing · Oklahoma City',
    stage: 'sourcing',
    stageIdx: 0,
    fit: 72,
    score: null,
  ...
  },
  {
    ...
    sub: 'Commercial electrical · Austin, TX',
    stage: 'rundown',
    stageIdx: 1,
    fit: 79,
    score: 64,
...
/* ═══════════════════════════════════════════════════════════════════
   Pinned artifacts (shown on mobile Today horizontal scroller)
   ═══════════════════════════════════════════════════════════════════ */

export const PINNED: PinnedArtifact[] = [
...
/* ═══════════════════════════════════════════════════════════════════
   Chat seeds — canned conversations per deal (or 'compare' view)
   ═══════════════════════════════════════════════════════════════════ */

export const CHAT_SEEDS: Record<string, ChatMessageSeed[]> = {
  atlas: [
...
/* ═══════════════════════════════════════════════════════════════════
   Yulia reply generator
   ═══════════════════════════════════════════════════════════════════ */

export interface SpawnTabSpec {
  kind: string;
  dealId?: string;
  dealIds?: string[];
  label: string;
...
export interface YuliaReply {
  text: string;
  cards: string[] | null;
  spawnTabs?: SpawnTabSpec[];
  progress?: string[];
}

export function yuliaReply(
  text: string,
  _deal: Deal | null,
  ...
  const mentioned = portDeals.filter(
    (d) =>
      t.includes(d.name.toLowerCase().split(' ')[0]) ||
      t.includes(d.id),
  );
  ...
  if (wantsModel && (mentioned.length >= 2 || (wantsCompare && mentioned.length === 0))) {
    const targets =
      mentioned.length >= 2
        ? mentioned
        : portDeals.filter((d) => d.score).slice(0, 3);
    const spawnTabs: SpawnTabSpec[] = targets.map((d) => ({
      kind: 'model',
      dealId: d.id,
      label: `${d.name} · DCF`,
    }));
    ...
    return {
      text: `Got it. Modeling DCF on <strong>${targets
        .map((d) => d.name)
        .join(', ')}</strong>${wantsCompare ? ' and opening a comparison' : ''}. Spawning ${spawnTabs.length} tabs — you can switch between them on the left.`,
      cards: null,
      spawnTabs,
      progress: spawnTabs.map((s) => s.label),
    };
  }
...
/* ═══════════════════════════════════════════════════════════════════
   Library — data rooms, my docs, shared docs, templates
   ═══════════════════════════════════════════════════════════════════ */

export const DATA_ROOMS: DataRoomItem[] = [
  // Atlas Air (DD in progress — biggest room)
  ...
  // Summit Climate (Rundown stage — smaller room)
  ...
  // Benchmark (LOI drafted — closing-stage docs)
  ...
  // Ridge (early sourcing — almost nothing yet)
  ...
  // Clearwater (rundown — walk-away flagged)
