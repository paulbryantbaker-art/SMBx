// @ts-nocheck — mechanical port of v4-library.jsx; visual-only,
// show-and-tell scope. Will re-type once the API wiring pass lands.
/* ═══════════════════════════════════════════════════════════
   LibraryView — port of claude_design/app/project/v4-library.jsx.
   Side-nav: All · Data rooms · My docs · Shared · Templates
   Data rooms nested Portfolio > Deal, with To review / Action / Sealed.
   ═══════════════════════════════════════════════════════════ */

import { useState, useMemo, type ReactNode } from 'react';
import { DEALS, DATA_ROOMS, MY_DOCS, SHARED_DOCS, TEMPLATES, type Deal, type Portfolio } from '../data';

const vlUseState = useState;
const vlUseMemo = useMemo;
...
function LibraryView({ portfolio, onOpenDeal }: LibraryViewProps) {
  ...
  const rooms     = DATA_ROOMS || [];
  const mine      = MY_DOCS || [];
  const shared    = SHARED_DOCS || [];
  ...
  return (
    <div className="vlib">
      ...
      <div className="vlib__main">
        <VLibToolbar
          section={section}
          roomDeal={roomDealId ? DEALS.find(d => d.id === roomDealId) : null}
        ...
        <div className="vlib__body">
          ...
          {section === 'rooms' && roomDealId && (
            <VLibRoom deal={DEALS.find(d => d.id === roomDealId)} rooms={rooms.filter(r => r.dealId === roomDealId)} query={query} onBack={() => setRoomDealId(null)} />
          )}
          {section === 'mine' && (
  ...
  return (
    <div className="vlib__scroll">
      <div className="vlib__shared">
        {rows.map(s => {
          const deal = s.dealId ? DEALS.find(d => d.id === s.dealId) : null;
