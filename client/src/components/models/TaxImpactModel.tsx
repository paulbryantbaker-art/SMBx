...
export default function TaxImpactModel({ tabId }: Props) {
  ...
  return (
    <div className="p-5 space-y-6 max-w-3xl mx-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div className="rounded-xl p-4" style={{ border: '1px solid #DDD9D1' }}>
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="pt-2" style={{ borderTop: '2px solid #D4714E' }}>
          ...
          <div className="rounded-xl p-4" style={{ border: '1px solid #DDD9D1' }}>
            ...
            {assetSale ? (
              ...
              <p className="text-xs" style={{ color: '#5e5d59' }}>Add asset allocations to compare.</p>
            )}
          </div>
        </div>
      )}
      ...
      {installment && (
        <div>
          <h3 className="text-[11px] font-bold uppercase tracking-wider mb-3" style={{ color: '#5e5d59' }}>Installment Sale (§453)</h3>
          <p className="text-xs mb-2" style={{ color: '#5e5d59' }}>
              ...
              <thead>
                <tr style={{ borderBottom: '2px solid #D4714E' }}>
                  {['Year', 'Payment', 'Taxable Gain', 'Tax Due'].map(h => (
                    <th key={h} style={{ padding: '4px 8px', textAlign: 'right', fontSize: 10, color: '#5e5d59', textTransform: 'uppercase' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
...
function Row({ label, value, bold, color }: { label: string; value: string; bold?: boolean; color?: string }) {
  return (
    <div className="flex justify-between">
      <span style={{ color: '#5e5d59' }}>{label}</span>
      <span className="tabular-nums" style={{ fontWeight: bold ? 700 : 500, color: color || '#1a1918' }}>{value}</span>
    </div>
  );
}
