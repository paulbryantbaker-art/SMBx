...
const ROLE_COLORS: Record<string, { bg: string; text: string; darkBg: string; darkText: string }> = {
  owner:       { bg: '#D4714E', text: '#fff', darkBg: '#ec9d78', darkText: '#fff' },
  attorney:    { bg: '#2563EB', text: '#fff', darkBg: '#3B82F6', darkText: '#fff' },
  cpa:         { bg: '#059669', text: '#fff', darkBg: '#10B981', darkText: '#fff' },
  broker:      { bg: '#D97706', text: '#fff', darkBg: '#F59E0B', darkText: '#1a1918' },
  lender:      { bg: '#7C3AED', text: '#fff', darkBg: '#8B5CF6', darkText: '#fff' },
  consultant:  { bg: '#6B7280', text: '#fff', darkBg: '#9CA3AF', darkText: '#1a1918' },
  counterparty:{ bg: '#DC2626', text: '#fff', darkBg: '#EF4444', darkText: '#fff' },
  auditor:     { bg: '#0891B2', text: '#fff', darkBg: '#06B6D4', darkText: '#1a1918' },
};

function RoleBadge({ role }: { role: string | null }) {
  if (!role) return null;
  const colors = ROLE_COLORS[role] || { bg: '#6B7280', text: '#fff', darkBg: '#9CA3AF', darkText: '#1a1918' };
  return (
    <span
...
export default function DealMessagesPanel({ dealId, currentUserEmail, onClose }: DealMessagesPanelProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-[#1a1918]">
      {/* Header */}
      <div className="shrink-0 flex items-center justify-between px-4 py-3 border-b border-[#e8e6dc] dark:border-[#3A3C3E]">
        <div className="flex items-center gap-2">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#D4714E" strokeWidth="2">
            <path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z" />
          </svg>
          <span className="text-sm font-semibold text-[#1a1918] dark:text-[#F5F3EF]">
            Deal Discussion
          </span>
        ...
        {onClose && (
          <button
            onClick={onClose}
            className="w-6 h-6 rounded hover:bg-[#F5F5F5] dark:hover:bg-[#141413] flex items-center justify-center cursor-pointer border-0 bg-transparent"
          >
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3 min-h-0">
        {loading && (
          <div className="flex justify-center py-8">
            <div className="w-6 h-6 border-2 border-[#D4714E] border-t-transparent rounded-full animate-spin" />
          </div>
        )}
          ...
          return (
            <div key={msg.id} className={`flex flex-col ${mine ? 'items-end' : 'items-start'}`}>
              ...
              <div className={`flex items-center gap-1.5 mb-1 ${mine ? 'flex-row-reverse' : ''}`}>
                <span className="text-[11px] font-semibold text-[#3d3d3a] dark:text-[#C8C4BC]">
              ...
              <div
                className={`max-w-[85%] rounded-xl px-3.5 py-2.5 text-sm leading-relaxed ${
                  mine
                    ? 'bg-[#D4714E] text-white rounded-br-sm'
                    : 'bg-[#F5F5F5] dark:bg-[#141413] text-[#1a1918] dark:text-[#E8E6E3] rounded-bl-sm'
                }`}
              >
                {msg.content}
              </div>
      ...
      {/* Compose */}
      <div className="shrink-0 px-4 py-3 border-t border-[#e8e6dc] dark:border-[#3A3C3E]">
        <div className="flex items-end gap-2">
          <textarea
            ...
            rows={1}
            className="flex-1 resize-none text-sm text-[#1a1918] dark:text-[#E8E6E3] bg-[#F5F5F5] dark:bg-[#141413] rounded-xl px-3.5 py-2.5 border-0 outline-none placeholder:text-[#A9A49C]"
