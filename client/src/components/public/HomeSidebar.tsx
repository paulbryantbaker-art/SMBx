          ...
          return (
            <div key={gate.id} className="flex items-center flex-1 min-w-0">
              ...
              <div className="flex flex-col items-center">
                <div
                  className={`w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold shrink-0 ${
                    isCompleted
                      ...
                      : isActive
                        ? 'border-2 border-[#BA3C60] text-[#BA3C60]'
                ...
                <span className={`text-[8px] mt-0.5 leading-tight text-center whitespace-nowrap ${isActive ? 'text-[#BA3C60] font-semibold' : 'text-[#A9A49C]'}`}>
                  {gate.name}
                </span>
              </div>
              {/* Connector line */}
              {i < gates.length - 1 && (
                <div className={`flex-1 h-px mx-0.5 mt-[-10px] ${isCompleted ? 'bg-[#BA3C60]' : 'bg-[rgba(0,0,0,0.08)]'}`} />
              )}
            </div>
          );
        })}
      ...
      >
          ...
          <div className="mx-3 my-2 p-3 bg-white rounded-xl" style={{ boxShadow: '0 1px 3px rgba(0,0,0,.06)' }}>
            ...
            {sessionData?.seven_factor_composite !== undefined && (
              <div className="flex items-center gap-1.5 mt-1.5">
                ...
                <span className="text-sm font-bold text-[#BA3C60]">{sessionData.seven_factor_composite}/70</span>
              </div>
            )}
        ...
        <div className="border-t border-[rgba(0,0,0,0.08)] px-4 py-3">
          <p className="text-[11px] text-[#A9A49C] m-0">
            Free preview · <a href="/signup" className="text-[#BA3C60] font-semibold no-underline hover:underline">Sign up</a> for full access
          </p>
        </div>
      </div>
    </>
