    ...
    <div className="min-h-dvh bg-[#f5f4ed]">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        <div className="mb-6">
          ...
          <p className="text-sm text-[#5e5d59] m-0 mt-1">Government data + AI synthesis</p>
        </div>

        {/* FRED Economic Indicators */}
        <div className="mb-8">
          <h2 className="text-sm font-semibold text-[#5e5d59] uppercase tracking-wide m-0 mb-3">Economic Indicators</h2>
          {fredLoading ? (
        ...
        {tab === 'overview' && (
          <div>
            <div className="bg-white rounded-2xl border border-border p-5 mb-6">
              <h3 className="text-base font-semibold text-[#1a1918] m-0 mb-4">Look Up a Market</h3>
              <div className="flex flex-col sm:flex-row gap-3">
                <div className="flex-1">
                  <label className="block text-xs font-medium text-[#5e5d59] mb-1">NAICS Code</label>
                  <input
                    type="text"
                    value={naicsCode}
                ...
                <div className="flex-1">
                  <label className="block text-xs font-medium text-[#5e5d59] mb-1">Geography (optional)</label>
                  <input
                    type="text"
                    value={geography}
                ...
                <div className="flex items-end">
                  <button
                    ...
                    className="px-5 py-2 rounded-xl text-sm font-semibold bg-[#D4714E] text-white border-0 cursor-pointer hover:bg-[#B85A3A] transition-colors disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
                  >
              ...
              <div className="space-y-4">
                {overview.cbpData && (
                  <div className="bg-white rounded-2xl border border-border p-5">
                    <h3 className="text-sm font-semibold text-[#1a1918] m-0 mb-3">Census Business Patterns</h3>
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                      {overview.cbpData.totalEstablishments !== undefined && (
                        <div>
                          ...
                          <p className="text-lg font-bold text-[#1a1918] m-0">{Number(overview.cbpData.totalEstablishments).toLocaleString()}</p>
                        </div>
                      )}
                      {overview.cbpData.totalEmployment !== undefined && (
                        <div>
                          <p className="text-[11px] text-[#A9A49C] m-0 mb-0.5">Total Employment</p>
                          <p className="text-lg font-bold text-[#1a1918] m-0">{Number(overview.cbpData.totalEmployment).toLocaleString()}</p>
                        </div>
                      )}
                      {overview.cbpData.totalPayroll !== undefined && (
                        <div>
                          <p className="text-[11px] text-[#A9A49C] m-0 mb-0.5">Total Annual Payroll</p>
                          <p className="text-lg font-bold text-[#1a1918] m-0">${Number(overview.cbpData.totalPayroll).toLocaleString()}</p>
                        </div>
                      )}
                    </div>
                        ...
                        <div className="space-y-1.5">
                          {overview.cbpData.sizeBreakdown.map((sz: any, i: number) => (
                            <div key={i} className="flex items-center gap-2">
                              <span className="text-xs text-[#5e5d59] w-32 shrink-0">{sz.label}</span>
                              ...
                              <span className="text-xs text-[#5e5d59] w-16 text-right">{sz.count?.toLocaleString()}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                ...
                {overview.sbaAnalysis && (
                  <div className="bg-white rounded-2xl border border-border p-5">
                    <h3 className="text-sm font-semibold text-[#1a1918] m-0 mb-3">SBA Bankability</h3>
            ...
            {!overview && !loading && (
              <div className="text-center py-12">
                ...
                <p className="text-base font-semibold text-[#1a1918] m-0 mb-1">Enter a NAICS code to explore</p>
                <p className="text-sm text-[#5e5d59] m-0">Get Census business patterns, SBA bankability, and economic indicators for any industry.</p>
              </div>
            )}
          </div>
        )}
          ...
          <div>
            {reportsLoading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-base font-semibold text-[#1a1918] m-0 mb-1">No reports yet</p>
                <p className="text-sm text-[#5e5d59] m-0">Generate a market overview to create your first intelligence report.</p>
              </div>
            ) : (
              <div className="space-y-3">
                {reports.map(r => (
                  <div key={r.id} className="bg-white rounded-2xl border border-border p-5 hover:border-[#D4714E] transition-colors">
                    ...
                    <h3 className="text-base font-semibold text-[#1a1918] m-0">{r.title}</h3>
                    <div className="flex items-center gap-3 mt-2 text-[12px] text-[#5e5d59]">
        ...
        {tab === 'sba' && (
          <div>
            <div className="bg-white rounded-2xl border border-border p-5 mb-6">
              <h3 className="text-base font-semibold text-[#1a1918] m-0 mb-4">SBA Loan Calculator</h3>
              <p className="text-sm text-[#5e5d59] m-0 mb-4">Check if a deal qualifies for SBA 7(a) financing. Uses live FRED prime rate data.</p>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
                <div>
                  <label className="block text-xs font-medium text-[#5e5d59] mb-1">Purchase Price ($)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs font-medium text-[#5e5d59] mb-1">Annual Debt Service ($)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-xs font-medium text-[#5e5d59] mb-1">EBITDA ($)</label>
                  <input
                    type="number"
            ...
            {sbaResult && (
              <div className="bg-white rounded-2xl border border-border p-5">
                <h3 className="text-sm font-semibold text-[#1a1918] m-0 mb-4">SBA Analysis Results</h3>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                  ...
                  {sbaResult.primeRate !== undefined && (
                    <div>
                      ...
                      <p className="text-xl font-bold text-[#1a1918] m-0">{sbaResult.primeRate}%</p>
                  ...
                  {sbaResult.estimatedRate !== undefined && (
                    <div>
                      ...
                      <p className="text-xl font-bold text-[#1a1918] m-0">{sbaResult.estimatedRate}%</p>
                      <p className="text-[10px] text-[#A9A49C] m-0">Prime + 2.75%</p>
                    </div>
                  )}
                </div>
                {sbaResult.notes && (
                  <p className="text-sm text-[#5e5d59] m-0 p-3 bg-[#f5f4ed] rounded-lg">{sbaResult.notes}</p>
                )}
              </div>
            )}
            ...
            {!sbaResult && !loading && (
              <div className="text-center py-8">
                <p className="text-sm text-[#5e5d59] m-0">Enter deal parameters above to check SBA 7(a) loan eligibility.</p>
              </div>
            )}
          </div>
        )}
