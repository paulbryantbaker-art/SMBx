...
export default function AdminTraffic() {
  ...
  return (
    <div>
      {/* KPI Cards */}
      <div className="grid grid-cols-2 gap-4">
        <div className="rounded-xl p-4 bg-white border border-[#e8e6dc]">
        ...
        <div className="rounded-2xl bg-white border border-[#e8e6dc] p-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead>
                <tr>
                  <th className="text-left text-[10px] uppercase tracking-widest font-bold text-[#5e5d59] pb-3">
                    Location
                  </th>
                  <th className="text-right text-[10px] uppercase tracking-widest font-bold text-[#5e5d59] pb-3">
                    Visitors
                  </th>
                </tr>
              </thead>
              <tbody>
                {locationCounts.length === 0 && (
                  <tr>
                    <td colSpan={2} className="text-sm text-[#5e5d59] py-4 text-center">
                      No geo data available
                    </td>
                  </tr>
                )}
                {locationCounts.map(([loc, count]) => (
                  <tr key={loc} className="border-t border-[#e8e6dc]">
                    <td className="text-sm text-[#1a1918] py-2">{loc}</td>
                    <td className="text-sm text-[#1a1918] py-2 text-right tabular-nums">
                      {count}
                    </td>
                  </tr>
                ))}
            ...
            <table className="w-full">
              ...
              <tbody>
                ...
                {referrerCounts.map(([source, count]) => (
                  <tr key={source} className="border-t border-[#e8e6dc]">
                    <td className="text-sm text-[#1a1918] py-2">{source}</td>
                    <td className="text-sm text-[#1a1918] py-2 text-right tabular-nums">
                      {count}
                    </td>
                  </tr>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {visitors.length === 0 && (
                <tr>
                  <td colSpan={5} className="text-sm text-[#5e5d59] py-4 text-center">
                    No visitor data
                  </td>
                </tr>
              )}
              {visitors.map((v) => {
                ...
                return (
                  <tr key={v.session_id} className="border-t border-[#e8e6dc]">
                    <td className="text-sm text-[#1a1918] py-2 font-mono">
                      {v.session_id.slice(0, 8)}
                    </td>
                    <td className="text-sm text-[#1a1918] py-2 max-w-[200px]">
                      <span className="truncate block">
                        {pagesShown.join(', ')}
                        {remaining > 0 && (
                          <span className="text-[#5e5d59]"> +{remaining} more</span>
                        )}
                      </span>
                    </td>
                    <td className="text-sm text-[#1a1918] py-2 whitespace-nowrap">
                      {v.geo?.city
                        ? `${v.geo.city}, ${v.geo.region}`
                        : '\u2014'}
                    </td>
                    <td className="text-sm text-[#5e5d59] py-2 whitespace-nowrap">
                      {timeAgo(v.last_seen)}
                    </td>
                    <td className="text-sm text-[#1a1918] py-2">
