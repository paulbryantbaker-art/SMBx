/* V3 — LearnDoc: combined How it works + Pricing document.
   ...
   compliance footnote. Section labels mirror v0.5 (§01 engine, §02
   real interaction, §03 compounding, §04 language, §05 pricing, §06 faq). */
    ...
    <div style={vL.docInner}>
      ...
      <LSec n="04" title="The language · three terms in every Yulia deliverable" eyebrow="THE LANGUAGE">
        ...
        <div style={vL.glossGrid}>
          <GlossCard
            term="The Baseline™"
            body="A multi-scenario valuation with implications. Three or four paths — each with price, structure, timing, tax, and close certainty defended against the seller's financials and the current market. The starting point for the conversation that decides the deal."
          />
          <GlossCard
            term="Blind Equity™"
            body="The value hiding in financials that an unprepared seller leaves behind. Owner-comp normalization. Defended add-backs. Working-capital pegging. Customer-concentration scoring. The QoE-style work that sets the floor on the seller's price — and surfaces the upside no one was scoring."
          />
          <GlossCard
            term="The Rundown™"
      ...
      <LSec n="05" title="Four tiers + enterprise · priced so you don't have to think about it" eyebrow="PRICING">
        ...
        <div style={vL.rulesBox}>
          ...
          {[
            ["Every paid tier delivers every core capability.", "Every hero feature is in every plan."],
            ["Subscription is the entire cost.", "Closed deal or broken. Predictable from day one."],
            ["Post-close support is included in every paid tier.", "180 days of PMI included; subscription continues as long as you do."],
            ["30-day free trial of Pro.", "14-day opt-out trial on every paid tier. Cancel anytime.", true],
            ["One-time $99 credit pack", "for a second deliverable when you're not yet ready for Starter."],
            ["Month-to-month flexibility at launch.", "Annual discount lands once retention data earns it."],
          ].map(([head, tail, accent], i) => (
            <div key={i} style={vL.ruleRow}>
              <span style={{ ...vL.ruleDot, ...(accent ? { background: "var(--accent)", boxShadow: "0 0 0 3px var(--accent-soft)" } : null) }} />
              <span style={{ fontSize: 12.5, color: "var(--ink-2)", lineHeight: 1.55 }}>
                <strong style={{ color: accent ? "var(--accent)" : "var(--ink)" }}>{head}</strong>{" "}{tail}
              </span>
            </div>
          ))}
        </div>
...
function LSec({ n, title, eyebrow, children }) {
  return (
