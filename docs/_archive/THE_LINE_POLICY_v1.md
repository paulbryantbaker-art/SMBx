> **SUPERSEDED (2026-07-11).** THE LINE v2 (`/THE_LINE_POLICY.md`) now governs — smbX
> pivoted from software product to a buy-side corp-dev-as-a-service practice, and v2
> retires this file's business-model absolutes ("no success fee ever, software-only").
> Retained for the record only; do not build against it. DEFINITIVE methodology and
> Yulia's behavioral guardrails are unaffected by the supersession.
